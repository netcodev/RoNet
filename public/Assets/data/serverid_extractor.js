...
window.addEventListener('ronet-extract-serverid-request', function(event) {
    const { extractionId } = event.detail;
    
    try {
        const element = document.querySelector(`[data-ronet-extraction-id="${extractionId}"]`);
        if (!element) {
            window.dispatchEvent(new CustomEvent('ronet-serverid-extracted', {
