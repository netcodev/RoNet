    ...
    chrome.storage.local.get(null, (currentSettings) => {
        ...
        for (const [key, defaultValue] of Object.entries(defaults)) {
            ...
            if (storedValue === undefined) {
                ...
                if (storedValue === null) {
                    console.warn(`RoNet: Setting '${key}' was null but expected ${defaultType}. Resetting.`);
                    settingsToUpdate[key] = defaultValue;
                    needsUpdate = true;
                } else if (storedType !== defaultType) {
                    console.warn(`RoNet: Type mismatch for '${key}'. Expected ${defaultType}, got ${storedType}. Resetting.`);
                    settingsToUpdate[key] = defaultValue;
                    needsUpdate = true;
                }
            }
        ...
        if (needsUpdate) {
            chrome.storage.local.set(settingsToUpdate, () => {
                if (chrome.runtime.lastError) {
                    console.error('RoNet: Failed to sync settings.', chrome.runtime.lastError);
                } else {
                    console.log(`RoNet: Synced/Fixed ${Object.keys(settingsToUpdate).length} settings (Trigger: ${reason}).`);
                }
            });
        }
    });
...
function updateUserAgentRule() {
    ...
    let ronetSuffix = `RoNetExtension(RoNet/${browser}/${engine}/${version}/${environment})`;
    if (engine === "Gecko" || engine === "WebKit") {
        ronetSuffix += " UnofficialRoNetVersion"; // If you are developing a port for either of these don't remove this. It tells Roblox that I don't control requests coming from your port.
    }
    ...
    const rules = [
        {
            ...
            action: {
                type: 'modifyHeaders',
                requestHeaders: [{ header: 'User-Agent', operation: 'set', value: `${originalUA} ${ronetSuffix}` }]
            },
            condition: { regexFilter: ".*_RoNetRequest=", resourceTypes: ["xmlhttprequest"] }
        },
        {
            id: 1000,
            priority: 10,
            action: {
                type: 'modifyHeaders',
                requestHeaders: [{ header: 'User-Agent', operation: 'set', value: `Roblox/WinInet ${ronetSuffix}` }]
            },
            condition: { regexFilter: "^https://gamejoin\\.roblox\\.com/.*_RoNetRequest=|^https://apis\\.roblox\\.com/player-hydration-service/v1/players/signed", resourceTypes: ["xmlhttprequest"] }
        }
    ];
...
const contextMenuClickListener = async (info, tab) => {
    if (info.menuItemId.startsWith('ronet-copy-universe-')) {
        const placeId = info.menuItemId.replace('ronet-copy-universe-', '');
    ...
    } else if (info.menuItemId.startsWith('ronet-copy-') && tab?.id) {
        const textToCopy = info.menuItemId.replace('ronet-copy-', '');
...
async function getUniverseIdFromPlaceId(placeId) {
    try {
        ...
        console.error('RoNet: Error fetching universe ID from place ID', e);
        return null;
    }
}
...
async function wearOutfit(outfitData) {
    ...
    try {
        ...
        if (!outfitId) {
            console.error('RoNet: wearOutfit called with invalid outfitData', outfitData);
            return { ok: false };
        }
        ...
        console.error('RoNet: Error wearing outfit', e);
        return { ok: false };
    }
}
    ...
    if (JSON.stringify(presence) !== JSON.stringify(state.latestPresence)) {
        ...
        if (isJoiningGame(presence) && presence.gameId && presence.rootPlaceId) {
            if (!isJoiningGame(oldPresence) || oldPresence.gameId !== presence.gameId) {
                chrome.storage.local.get({ 'ronet_server_history': {} }, (res) => {
                    const history = res.ronet_server_history || {};
                    ...
                    chrome.storage.local.set({ 'ronet_server_history': history });
                });
            }
        }
    }
...
function updateAvatarRotator() {
    chrome.storage.local.get(['ronet_avatar_rotator_enabled', 'ronet_avatar_rotator_ids', 'ronet_avatar_rotator_interval'], (data) => {
        if (state.rotatorInterval) {
            clearInterval(state.rotatorInterval);
            state.rotatorInterval = null;
        }

        if (data.ronet_avatar_rotator_enabled && data.ronet_avatar_rotator_ids?.length > 0) {
            const ids = data.ronet_avatar_rotator_ids;
            state.rotatorIndex = 0;
            
            let intervalSeconds = Math.max(parseInt(data.ronet_avatar_rotator_interval, 10) || 5, 5);
...
chrome.storage.onChanged.addListener((changes, namespace) => {
    if (namespace === 'local') {
        ...
        if (changes.ronet_avatar_rotator_enabled || changes.ronet_avatar_rotator_ids || changes.ronet_avatar_rotator_interval) {
            updateAvatarRotator();
        }
    }
});
    ...
    switch (request.action) {
        ...
        case 'requestPermission':
            chrome.permissions.request({ permissions: [].concat(request.permission) }, (granted) => {
                if (chrome.runtime.lastError) console.warn("RoNet: Permission request failed:", chrome.runtime.lastError);
                                ...
                                if (item.type === 'Universe') {
                                    if (settings.copyUniverseIdEnabled) {
                                        chrome.contextMenus.create({
                                            id: `ronet-copy-universe-${item.id}`,
