...
const UI = {
    createFilterPanel(onFilterChange) {
        ...
        container.className = 'ronet-filters-container';

        const createFilterSection = (label, element) => {
            const div = document.createElement('div');
            div.className = 'ronet-filter-section';
    ...
    createEmptyState(onClick) {
        const container = document.createElement('div');
        container.className = 'ronet-empty-state section';
...
class HiddenGamesManager {
    ...
    openOverlay() {
        ...
        list.classList.add('ronet-hidden-games-list');

        const loader = document.createElement('div');
        loader.className = 'ronet-load-more-container';
    ...
    async loadMore() {
        ...
        this.elements.loader.innerHTML = `<p class="ronet-loading-text">Loading...</p>`;
            ...
            contents.forEach((content) => {
                ...
                if (content.children.length === 1) {
                    ...
                    if (inner.tagName === 'DIV' && inner.children.length === 0 && inner.textContent.trim() === '') {
                        if (content.querySelector('.ronet-empty-state')) return;
                        content.innerHTML = '';
