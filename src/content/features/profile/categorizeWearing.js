...
// TODO make roseals currently wearing replace ronets.
let totalPrice = 0;
...
async function loadAssetTypeIds() {
    try {
        ...
        console.error('RoNet: Failed to load dynamic asset type IDs', e);
    }
}
...
function updateTotalDisplay() {
    if (!totalPriceElement) {
        totalPriceElement = document.getElementById('ronet-wearing-total-price');
    }
    if (totalPriceElement) {
        if (totalPrice > 0) {
...
function recalculateTotalPrice() {
    ...
    const allCards = document.querySelectorAll('.ronet-category-grid .ronet-item-card');
    
    allCards.forEach(card => {
        if (card.classList.contains('shimmer')) return;

        const link = card.querySelector('a.ronet-item-card-link');
        if (!link) return;
        ...
        const price = parseFloat(card.dataset.ronetPrice);
        if (isNaN(price) || price === 0) {
            return;
        }

        const bundleId = card.dataset.ronetBundleId;
...
function updateScrollButtonStates(container, leftBtn, rightBtn) {
    ...
    leftBtn.classList.toggle('ronet-btn-disabled', scrollLeft <= 5);
    const isAtEnd = scrollLeft + clientWidth >= scrollWidth - 5;
    rightBtn.classList.toggle('ronet-btn-disabled', isAtEnd);
}
...
function refreshPillToggle() {
    ...
    const newPillToggle = createPillToggle({
        ...
        onChange: (value) => {
            currentFilter = value;
            updateTabVisibility();
            const container = document.querySelector('.ronet-items-scroll-container');
            if (container) {
...
function createCategorizedWearingSection() {
    ...
    section.className = 'section ronet-container'; 
    ...
    totalPriceElement.id = 'ronet-wearing-total-price';
    totalPriceElement.style.cssText = 'display: flex; align-items: center; color: var(--ronet-secondary-text-color); font-size: 16px;';

    const spacer = document.createElement('div');
    spacer.style.flexGrow = '1';
    
    pillToggleWrapper = document.createElement('div');
    pillToggleWrapper.className = 'ronet-pill-wrapper';
    header.append(title, totalPriceElement, spacer, pillToggleWrapper);
    
    const scrollWrapper = document.createElement('div');
    scrollWrapper.className = 'ronet-scroll-wrapper';
    scrollWrapper.style.cssText = 'position: relative; width: 100%; display: flex; align-items: center;';

    const scrollContainer = document.createElement('div');
    scrollContainer.className = 'ronet-items-scroll-container';
    ...
    accessoriesGrid.className = 'ronet-category-grid wearables';
    ...
    if (isEmotesCategoryEnabled) {
        ...
        emotesGrid.className = 'ronet-category-grid emotes';
    ...
    if (isAnimationsCategoryEnabled) {
        ...
        animationsGrid.className = 'ronet-category-grid animations';
    ...
    if (isBodyPartsCategoryEnabled) {
        ...
        bodyPartsGrid.className = 'ronet-category-grid body-parts';
...
function addItemToCategoryView(itemEl, assetId) {
    if (itemEl.dataset.ronetCategorized === 'true') return;
    const info = assetInfoCache.get(assetId);
    ...
    const exists = Array.from(targetGrid.children).some(child => 
        child.dataset.ronetPendingId == assetId || 
        child.querySelector(`a[href*="/${assetId}/"]`)
    );

    if (!exists) {
        itemEl.dataset.ronetCategorized = 'true';
        const card = createItemCard(assetId, {}, { cardStyles: { width: '150px', flexShrink: 0 } });
        card.dataset.ronetPendingId = assetId;
...
export async function init() {
    ...
    observeElement('.profile-tab-content', (content) => {
        if (document.getElementById('ronet-main-categorized-wrapper')) return;
        ...
        if (originalWearing) {
            ...
            categorizedSection.id = 'ronet-main-categorized-wrapper';
    ...
    observeElement('.profile-currently-wearing, .roseal-currently-wearing', (wearing) => {
        ...
        if (!document.getElementById('ronet-main-categorized-wrapper')) {
             const categorizedSection = createCategorizedWearingSection();
             categorizedSection.id = 'ronet-main-categorized-wrapper';
    ...
    window.addEventListener('ronet-catalog-details', (e) => {
    ...
    hideStyle.innerHTML = `
        ...
        .ronet-items-scroll-container::-webkit-scrollbar { display: none; }
        .ronet-scroll-btn {
        ...
        .ronet-scroll-btn.left { left: 5px; }
        .ronet-scroll-btn.right { right: 5px; }
        .ronet-scroll-btn.ronet-btn-disabled { opacity: 0.25 !important; cursor: default; pointer-events: auto; }
    `;
    ...
    selectors.forEach(selector => {
        observeElement(selector, (item) => {
            if (!item.dataset.ronetProcessed) {
                item.dataset.ronetProcessed = "true";
                handleItemDetection(item);
            }
                    const debouncedRecalculate = debounce(recalculateTotalPrice, 250);

        observeElement('.ronet-category-grid .ronet-item-card', debouncedRecalculate, {
            multiple: true,
