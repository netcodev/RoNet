...
const rapDisplayIdentifier = 'ronet-user-rap-display';


export function getOrCreateRonetContainer(observedElement) {
    const CONTAINER_ID = 'ronet-profile-button-container';
    ...
    let ronetContainer = parentToCheck.querySelector(`.${CONTAINER_ID}`);

    if (!ronetContainer) {
        ronetContainer = document.createElement('div');
        ronetContainer.className = CONTAINER_ID;

        if (isNewLayout) {
            Object.assign(ronetContainer.style, {
                display: 'flex',
                gap: '8px',
                justifyContent: 'flex-start',
                marginTop: '12px' 
            });
            observedElement.appendChild(ronetContainer);
        } else {
            Object.assign(ronetContainer.style, {
                display: 'flex',
                gap: '8px',
            ...
            observedElement.insertAdjacentElement('afterend', ronetContainer);
        }
    }
    return ronetContainer;
}
    ...
    try {
        do {
            ...
            if (response.status === 429) { 
                ...
                console.warn(`RoNet (RAP): Rate limited. Retrying after ${retryAfter} seconds.`);
        ...
        console.error('RoNet: Failed to fetch user collectibles:', error);
        return null;
    }
}
...
async function showInventoryOverlay(userId, items, totalRapString, hideSerial) {
    ...
    const loadMoreItems = async () => {
        ...
        try { 
            ...
            if (error.name !== 'AbortError') { 
                console.error("RoNet: Failed to fetch item thumbnails.", error); 
            } 
        } finally { 
    ...
    itemListContainer.className = 'ronet-inventory-list';
    ...
    rolimonsLink.style.cssText = 'display: inline-flex; align-items: center; margin-left: 12px; color: var(--ronet-secondary-text-color);'; // Verified
    ...
    const actualTitleElement = overlay.querySelector('.ronet-overlay-header');
    ...
    const scrollTarget = overlay.querySelector('.ronet-overlay-body');
...
async function addUserRapDisplay(observedElement) {
    const targetContainer = getOrCreateRonetContainer(observedElement);
    ...
    robuxIcon.className = 'icon-robux-16x16 ronet-dynamic-icon';
    ...
    const rapDisplay = createProfileHeaderButton({
        ...
        textColor: 'var(--ronet-main-text-color)' 
    });
