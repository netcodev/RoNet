...
class HiddenGamesManager {
    ...
    render() {
        ...
        const createFilterGroup = (label, input) => el("div", "", { style: { display: "flex", flexDirection: "column", gap: "4px" } }, [el("label", "", { textContent: label, style: { fontSize: "12px", fontWeight: "500", color: "var(--ronet-secondary-text-color)" } }), input]);

        const body = el("div", "", { style: { display: "flex", flexDirection: "column" } }, [
            el(
                "div",
                "ronet-filters-container",
                {
            ...
            el("div", "ronet-load-more-container", { style: { padding: "10px 0", textAlign: "center" } }),
        ]);

        this.elements.list = body.querySelector(".hidden-games-list");
        this.elements.filters = body.querySelector(".ronet-filters-container");
        this.elements.loader = body.querySelector(".ronet-load-more-container");
        ...
        const scrollContainer = overlay.querySelector(".ronet-overlay-body");
        if (scrollContainer) {
    ...
    async loadMore() {
        ...
        this.elements.loader.innerHTML = DOMPurify.sanitize(`<p class="ronet-loading-text">Loading more games...</p>`);
        ...
        try {
            ...
            console.warn("RoNet: Error loading more games", err);
        } finally {
            this.elements.loader.innerHTML = "";
            this.isPaginating = false;
        }
...
export function init() {
    chrome.storage.local.get(["groupGamesEnabled"], (result) => {
        ...
        const cleanupLegacyButtons = (header) => {
            if (!header) return;
            const selectors = [".ronet-hidden-games-container", ".hidden-games-button"];
        ...
        const initHeader = async (header) => {
            ...
            const attachedGroupId = header.dataset.ronetGroupId;

            if (attachedGroupId) {
                if (attachedGroupId === currentGroupId) {
                    const existing = header.querySelectorAll(".ronet-hidden-games-container");
                    if (existing.length > 1) {
                        cleanupLegacyButtons(header);
                        delete header.dataset.ronetGroupId;
            ...
            header.dataset.ronetGroupId = currentGroupId;
