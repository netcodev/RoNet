...
function injectResultElement(targetElement, result) {
    ...
    let estimatorRow = pendingRow.parentNode.querySelector('.ronet-estimator-row');
    ...
    if (estimatorRow) {
        ...
        estimatorRow.className = 'ronet-estimator-row';
        
        estimatorRow.innerHTML = `
            <td>
                <div style="display: flex; align-items: center; color: var(--ronet-main-text-color);">
...
async function onElementFound(targetElement) {
    if (targetElement.dataset.ronetProcessed) return;
    targetElement.dataset.ronetProcessed = 'true';
