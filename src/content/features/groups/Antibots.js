...
async function showActionOverlay(selectedMembers, isFromBotScan = false) {
    ...
    bodyContainer.innerHTML = DOMPurify.sanitize(`
        <div class="ronet-action-description">Review ${selectedMembers.length} selected members:</div>
        <ul class="ronet-action-summary-list">
            ${selectedMembers.map(m => `<li>${m.user.displayName} (@${m.user.username})</li>`).join('')}
        </ul>
        ${isFromBotScan ? `<div class="ronet-action-bot-warning ronet-ban-warning"><strong>Warning:</strong> Ensure these are bots before taking action.</div>` : ''}
        <div class="ronet-action-permission-warning ronet-ban-warning" style="display:none"></div>
        <div class="ronet-action-progress-container" style="display:none">
            <div class="ronet-action-status-text"></div>
            <div class="ronet-action-progress-bar-container">
                <div class="ronet-action-progress-bar"></div>
            </div>
        </div>
    `);
    ...
    const warningEl = bodyContainer.querySelector('.ronet-action-permission-warning');
    ...
    btnCancel.onclick = () => {
        ...
        window.ronet.ui.actionOverlay = null;
    };

    window.ronet.ui.actionOverlay = overlayInstance;
    
    btnCancel.classList.add('ronet-action-cancel-btn');
    btnBan.classList.add('ronet-action-ban-btn');
    btnKick.classList.add('ronet-action-kick-btn');
}
...
function showDoubleConfirmOverlay(actionType, isFromBotScan) {
    ...
    if (actionType === 'ban' && isFromBotScan) {
        message += `<br><br><div class="ronet-ban-warning" style="margin-top:0;"><strong>Reminder:</strong> Verify bot scores before banning.</div>`;
    }

    bodyContent.innerHTML = DOMPurify.sanitize(`<div id="ronet-double-confirm-message" style="text-align:center; font-size:16px;">${message}</div>`);
    ...
    window.ronet.ui.doubleConfirmOverlay = overlayInstance;
...
async function processMembers(actionType) {
    ...
    const overlayInstance = window.ronet.ui.actionOverlay;
    ...
    const progressBar = overlayEl.querySelector('.ronet-action-progress-bar');
    const statusText = overlayEl.querySelector('.ronet-action-status-text');
    const progressContainer = overlayEl.querySelector('.ronet-action-progress-container');
    const summaryList = overlayEl.querySelector('.ronet-action-summary-list');
    const warning = overlayEl.querySelector('.ronet-action-bot-warning');
    const btns = overlayEl.querySelectorAll('button:not(.ronet-action-cancel-btn)'); 
    ...
    if (progressContainer) {
        ...
        const barContainer = progressContainer.querySelector('.ronet-action-progress-bar-container');
    ...
    for (let i = 0; i < totalMembers; i++) {
        if (!window.ronet.ui.actionOverlay || !document.body.contains(overlayEl)) return;
    ...
    const cancelBtn = overlayEl.querySelector('.ronet-action-cancel-btn');
    if(cancelBtn) {
...
function updateActionCount() {
    ['ronet-quick-ban-list', 'ronet-bot-list'].forEach(listId => {
        const container = document.getElementById(listId);
        if (!container || container.offsetParent === null) return; 

        const btnId = listId === 'ronet-quick-ban-list' ? 'ronet-quick-ban-confirm-btn' : 'ronet-antibot-confirm-btn';
        ...
        if (container && btn) {
            ...
            const selectAllBtnId = listId === 'ronet-quick-ban-list' ? 'ronet-quick-ban-select-all-btn' : 'ronet-antibot-select-all-btn';
...
async function addFeatureButtons(searchContainer) {
    if (document.getElementById('ronet-button-container')) return;
    ...
    buttonContainer.id = 'ronet-button-container';
    ...
    if (settings.antibotsEnabled) {
        antiBotsButton = createBtnControl('ronet-anti-bots-btn', 'Anti Bots');
        banByScoreButton = createBtnControl('ronet-ban-by-score-btn', 'Ban by Score', 'btn-alert-sm', true);
        antiBotSelectAllButton = createBtnControl('ronet-antibot-select-all-btn', 'Select All', '', true);
        antiBotConfirmButton = createBtnControl('ronet-antibot-confirm-btn', 'Action (0)', 'btn-alert-sm', true);
    ...
    if (settings.QuickActionsEnabled) {
        quickBanButton = createBtnControl('ronet-quick-ban-btn', 'Quick Action', 'btn-secondary-sm');
        quickBanSelectAllButton = createBtnControl('ronet-quick-ban-select-all-btn', 'Select All', '', true);
        quickBanConfirmButton = createBtnControl('ronet-quick-ban-confirm-btn', 'Action (0)', 'btn-alert-sm', true);
    ...
    if (quickBanButton) {
        quickBanButton.onclick = async function() {
            ...
            quickBanListContainer.id = 'ronet-quick-ban-list';
    ...
    async function loadNextQuickBanPage() {
        ...
        let loader = quickBanListContainer.querySelector('.ronet-loading-more');
        if (!loader) {
            loader = document.createElement('div');
            loader.className = 'ronet-loading-more';
    ...
    if (antiBotsButton) {
        antiBotsButton.onclick = async function() {
            ...
            botMemberListContainer.id = 'ronet-bot-list';
    ...
    if (banByScoreButton) {
        banByScoreButton.onclick = () => {
            ...
            const bodyContainer = document.createElement('div'); bodyContainer.innerHTML = DOMPurify.sanitize(`
                <div class="ronet-score-selector-container">
                    <label>Minimum Bot Score: <span id="ronet-score-value">50</span></label>
                    <input type="range" id="ronet-score-slider" class="ronet-slider" min="20" max="100" value="50">
                    <div class="ronet-score-impact-text">
                        This will select approximately <span id="ronet-score-ban-count">0</span> members.
                    </div>
                </div>
            `);
            ...
            const slider = bodyContainer.querySelector('#ronet-score-slider');
            const valDisplay = bodyContainer.querySelector('#ronet-score-value');
            const countDisplay = bodyContainer.querySelector('#ronet-score-ban-count');
...
function handleContainerRemoval() {
    document.getElementById('ronet-button-container')?.remove();
    document.getElementById('ronet-button-spacer')?.remove();
}

function antiBotsFullCleanup() {
    handleContainerRemoval();
    const overlays = document.querySelectorAll('.ronet-global-overlay');
    overlays.forEach(o => o.remove());
    
    document.getElementById('ronet-quick-ban-list')?.remove();
    document.getElementById('ronet-bot-list')?.remove();
...
export function init() {
    ...
    if (!window.ronet) window.ronet = {};
    if (!window.ronet.ui) window.ronet.ui = {};
