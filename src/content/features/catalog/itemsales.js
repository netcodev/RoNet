    ...
    chrome.storage.local.get({ itemSalesEnabled: false }, async (settings) => {
        ...
        if (!cachedItemsData) {
            try {
                ...
                console.error("RoNet: Failed to load items.json", e);
                return;
            }
        }
        ...
        observeElement('.price-row-container', (foundElement) => {
            ...
            if (foundElement.dataset.ronetItemSalesInjected === String(itemId)) return;

            foundElement.parentNode.querySelector('.ronet-item-sales-container')?.remove();

            const container = document.createElement('div');
            container.className = 'ronet-item-sales-container';
            ...
            foundElement.dataset.ronetItemSalesInjected = String(itemId);
        });
    });
}
