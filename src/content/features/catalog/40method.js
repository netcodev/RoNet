...
async function fetchTemplateBlobViaBatch() {
    const batchResponse = await callRobloxApi({
        ...
        body: [
            {
                "requestId": "ronet_req_" + Date.now(),
                "assetId": RONET_TEMPLATE_ASSET_ID,
                "type": "Place", 
                "format": "rbxl"
            }
        ]
...
async function publishTemplateToPlace(targetPlaceId) {
    try {
        ...
        console.error('RoNet: Auto-publish failed', error);
        throw error;
    }
}
...
async function fetchCatalogMetadata() {
    ...
    metadataPromise = (async () => {
        try {
            ...
            console.warn('RoNet: Failed to fetch catalog metadata', error);
        } finally {
...
async function updateGameDescription(universeId, sourcePlaceId) {
    try {
        ...
        console.warn('RoNet: Failed to update game description', e);
    }
}
...
async function validateGameSync(universeId, placeId) {
    try {
        const versionResponse = await callRobloxApiJson({
            ...
            method: 'POST',
            body: {
                assetIds: [parseInt(RONET_PLACE_ID)],
                versionStatus: 'Published'
            }
        });
        ...
        if (sourceId !== RONET_PLACE_ID || currentVersion < latestVersion) {
...
const checkItemOwnership = async (userId, itemId, itemType) => {
    try {
        ...
        console.warn('RoNet: Could not check item ownership:', error);
        return false; 
    }
};
...
async function fetchGamePassesForUniverse(universeId) {
    ...
    try {
        ...
        console.warn('RoNet: Failed to fetch game passes via API', error);
    }
    return gamePasses;
}
...
export const createAndShowPopup = (onSave, initialState = null) => {
    ...
    bodyContent.innerHTML = DOMPurify.sanitize(`
        <div id="sr-view-main">
            ...
            <button id="sr-use-ronet-group-btn" class="btn-secondary-md btn-min-width" style="width: 100%;">Donate Saved Robux to RoNet</button>
        ...
        <div id="sr-view-no-group-info" class="sr-hidden">
            ...
            <p class="text font-body" style="margin: 5px 0 10px 0; line-height: 1.5;">If you don't have a suitable group, you can instead support RoNet by using our experience to process the purchase, which will give RoNet the saved Robux ❤️</p>
        ...
        <div id="sr-view-ronet-group" class="sr-hidden">
            <h4 class="text font-header-2" style="margin: 0 0 10px 0;">Donate to RoNet</h4>
            <p class="text font-body" style="margin: 5px 0 10px 0; line-height: 1.5;"><strong>How it works:</strong> Your purchase will go through a game owned by RoNet, and RoNet will earn a commission on your purchase which will help support RoNet's development.</p>
            ...
            <ul class="text font-body" style="margin: 0 0 10px 0; padding-left: 20px; line-height: 1.5;">
                <li>The saved Robux will go to RoNet to help fund development</li>
                <li>You still get the item you're purchasing</li>
                <li>And you will support RoNet at no extra cost for you.</li>
            </ul>
            <div style="display: flex; gap: 8px; margin-top: 16px;">
                <button class="btn-secondary-md btn-min-width" id="sr-ronet-back-btn" style="flex: 1;">Back</button>
                <button class="btn-cta-md btn-min-width" id="sr-ronet-confirm-btn" style="flex: 1;">I Understand & Continue</button>
            </div>
        </div>
    ...
    const viewRoNetGroup = bodyContent.querySelector('#sr-view-ronet-group');
    ...
    const useRoNetGroupBtn = bodyContent.querySelector('#sr-use-ronet-group-btn');
    const ronetBackBtn = bodyContent.querySelector('#sr-ronet-back-btn');
    const ronetConfirmBtn = bodyContent.querySelector('#sr-ronet-confirm-btn');
    ...
    const safeSaveSettings = (placeId, useGroup, onSuccess) => {
        if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
            chrome.storage.local.set({
                ...
                useRoNetGroup: useGroup
            }, () => {
                if (chrome.runtime.lastError) {
                    console.error('RoNet: Storage save error:', chrome.runtime.lastError);
            ...
            console.error('RoNet: Storage API unavailable.');
    ...
    const showValidationWarning = async (reason, placeId, universeId, gameName = null) => {
        ...
        if (reason === 'outdated') {
            ...
            } catch (e) { console.error("RoNet: Failed to fetch initial place version", e); }
    ...
    const loadGroups = async () => {
        try {
            ...
            console.error('RoNet: Failed to fetch groups:', error);
            groupDropdownContainer.innerHTML = DOMPurify.sanitize('<div class="text font-body" style="color: var(--ronet-secondary-text-color);">Failed to load groups. Please refresh and try again.</div>');
        }
    };
...
createNewGameBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            console.log(`RoNet: Created Universe ${newUniverseId}, Place ${newPlaceId}`);
            ...
            await updateGameDescription(newUniverseId, RONET_PLACE_ID);
            ...
            console.error('RoNet: Create Game Error', error);
            if (errorEl) {
    ...
    if (editGameLink) {
        editGameLink.addEventListener('click', async (e) => {
            e.preventDefault();
            await launchStudioForGame(RONET_PLACE_ID);
        });
    }
    ...
    manualCreateDoneBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (newGames.length === 1) {
                ...
                await updateGameDescription(newGame.id, RONET_PLACE_ID);
    ...
    useRoNetGroupBtn.addEventListener('click', () => {
        viewMain.classList.add('sr-hidden');
        saveBtn.style.display = 'none';
        viewRoNetGroup.classList.remove('sr-hidden');
    });

    ronetBackBtn.addEventListener('click', () => {
        viewRoNetGroup.classList.add('sr-hidden');
        viewMain.classList.remove('sr-hidden');
        saveBtn.style.display = '';
    });
    ...
    updateConfirmBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (manualUniverseIdCandidate) {
                await updateGameDescription(manualUniverseIdCandidate, RONET_PLACE_ID);
                try {
                    await callRobloxApiJson({
                        subdomain: 'develop',
            ...
            console.error('RoNet: Update failed', e);
    ...
    ronetConfirmBtn.addEventListener('click', async () => {
        safeSaveSettings('RONET_GROUP', true, async () => {
            close();
            await showInitialConfirmation('RONET_GROUP', true);
            onSave();
        });
    });
    ...
    bodyContent.innerHTML = DOMPurify.sanitize(`
        <div style="padding: 20px; text-align: center;">
            ...
            <p class="text font-body" style="margin: 0; color: #666;">
                Please try again or report it in the RoNet Discord server if the issue persists.
            </p>
        </div>
    `);
...
const showInitialConfirmation = async (savedPlaceId, useRoNetGroup) => {
    const currentUserId = getCurrentUserId();
    ...
    if (useRoNetGroup || savedPlaceId === 'RONET_GROUP') {
        actualPlaceId = RONET_PLACE_ID;
        gameName = 'RoNet Group Experience';
    }
    ...
    try {
        ...
        console.warn('RoNet: Could not fetch game details:', error);
    }

    const isDonating = (useRoNetGroup || savedPlaceId === 'RONET_GROUP');
    ...
    confirmBody.innerHTML = DOMPurify.sanitize(`
        <div style="padding: 16px 0; margin-bottom: 16px; text-align: center; border-bottom: 1px solid rgb(73, 77, 90);">
            ...
            ${isDonating ? '<div class="text font-body" style="margin-top: 4px;">❤️ Donating to RoNet ❤️</div>' : ''}
        </div>
        <div style="padding: 12px 0; margin-bottom: 16px; border-bottom: 1px solid rgb(73, 77, 90);">
            ...
            <div style="display: flex; gap: 12px; align-items: center;">
                ...
                <div style="flex: 1; min-width: 0;">
                    ...
                    ${(useRoNetGroup || savedPlaceId === 'RONET_GROUP') ? '<div class="text font-body" style="font-size: 12px;">Donating to RoNet ❤️</div>' : ''}
                </div>
            </div>
        </div>
        <div style="padding: 12px 0; margin-bottom: 16px; border-bottom: 1px solid rgb(73, 77, 90);">
            ...
            <ol class="text font-body" style="margin: 0; padding-left: 20px;">
                ...
                <li>The item goes to you, ${(useRoNetGroup || savedPlaceId === 'RONET_GROUP') ? 'and RoNet will earn a commission on your purchase which will help support the extension, at no extra cost for you.' : 'but 40% of the Robux goes to your group'}</li>
                ${(useRoNetGroup || savedPlaceId === 'RONET_GROUP') ? '' : '<li>Robux will be pending for ~1 month</li>'}
            </ol>
        </div>
...
const executeCartPurchase = async (cartItems, prefetchData = null, bypassValidation = false) => {
    ...
    try {
        ...
        console.warn('RoNet: Could not fetch cart item thumbnails:', error);
    }
    ...
    if (prefetchData && prefetchData.storage) {
        ...
        result = await new Promise((resolve) => {
            chrome.storage.local.get(['RobuxPlaceId', 'useRoNetGroup'], resolve);
        });
    }

    const savedPlaceId = result.RobuxPlaceId;
    const useRoNetGroup = result.useRoNetGroup === true;
    ...
    if (prefetchData && prefetchData.gameInfo) {
        ...
        } catch (e) { console.warn('RoNet: Prefetch game info error', e); }
    } else {
        try {
            const gameData = await callRobloxApiJson({
                subdomain: 'games',
    ...
    if (!useRoNetGroup && savedPlaceId !== 'RONET_GROUP' && universeId && !bypassValidation) {
    ...
    finalConfirmBody.innerHTML = DOMPurify.sanitize(`
        <div style="padding: 12px 0 8px; text-align: center; border-bottom: 1px solid rgb(73, 77, 90);">
            ...
            ${isDonating ? '<div class="text font-body" style="margin-top: 4px; font-size: 12px;">❤️ Donating to RoNet ❤️</div>' : ''}
        </div>
        ...
        <details style="border-bottom: 1px solid rgb(73, 77, 90); padding: 8px 0;">
            ...
            <div style="padding-top: 8px; display: flex; gap: 10px; align-items: center;">
                ...
                <div style="flex: 1; min-width: 0;">
                    ...
                    ${isDonating ? '<div class="text font-body" style="font-size: 11px; opacity: 0.7;">Donating to RoNet ❤️</div>' : ''}
                </div>
            </div>
        </details>
        <div style="padding: 8px 0;">
            ...
            <div style="display: flex; justify-content: space-between; padding: 8px; background: rgba(0,128,0,0.05); border-radius: 4px;">
                <span class="text font-body" style="font-weight: 600; font-size: 13px;">${isDonating ? 'RoNet gets:' : 'You Save:'}</span>
    ...
    const placeIdToUse = (useRoNetGroup || savedPlaceId === 'RONET_GROUP') ? actualPlaceId : savedPlaceId;
    ...
    if (itemId && ((!itemDetails || !itemDetails.thumbnail) || (!isGamePass && !isBundle))) {
        if (isGamePass) {
            try {
                ...
                console.warn('RoNet: Could not fetch game pass thumbnail:', error);
            }
        } else {
            try {
                ...
                console.warn('RoNet: Could not fetch item details:', error);
            }
        }
    }
...
const addSaveButton = (modal) => {
    ...
    async function addButtonWithElements({ buyNowButton, robuxPriceElement, buttonContainer, closeButton }) {
        ...
        if (currentUserId) {
            prefetchData.storage = new Promise(resolve => chrome.storage.local.get(['RobuxPlaceId', 'useRoNetGroup'], resolve));
            ...
            prefetchData.gameInfo = prefetchData.storage.then(async (res) => {
                ...
                const useRoNetGroup = res.useRoNetGroup === true;
                ...
                try {
                    ...
                    return { data, actualPlaceId, useRoNetGroup, savedPlaceId };
                } catch (e) { return null; }
            });
        ...
        if (!isGamePass && !isBundle && !isMultiItemPurchase && itemId) {
            try {
                ...
                console.warn('RoNet: Failed to fetch asset type for button text', e);
            }
        }
            ...
            if (isMismatch) {
                ...
                errorBody.innerHTML = DOMPurify.sanitize(`
                    <div style="padding: 20px; text-align: center;">
                        ...
                        <p class="text font-body" style="margin: 0;">Please refresh the page and try again. If this issue persists, please report it in the RoNet Discord server.</p>
                    </div>
                `);
                const { overlay, close } = createOverlay({
            ...
            if (isGamePassOnGamePage) {
                ...
                try {
                    ...
                    try {
                        ...
                        console.warn('RoNet: Could not fetch item thumbnail:', thumbError);
                    }
                    ...
                    console.warn('RoNet: Could not extract item details from page:', error);
                }
            }
        ...
        if (!modalWindow.querySelector('.ronet-save-wrapper')) {
            const wrapper = document.createElement('div');
            wrapper.className = 'ronet-save-wrapper';
...
export function init() {
    if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
        ...
        console.error('RoNet: Chrome storage API not available.');
    }
}
