...
export function init() {
    chrome.storage.local.get({ ParentItemsEnabled: false }, async (settings) => {
        ...
        const banner = document.getElementById('ronet-catalog-notice-banner');
        if (banner) {
            banner.innerHTML = '';
            banner.removeAttribute('style');
            banner.classList.remove('ronet-banner-compact');
        }
        ...
        try {
            ...
            if (data && data.data && data.data.length > 0) {
                if (data.data.length > 1) {
                    ...
                    observeElement('#ronet-catalog-notice-banner', (banner) => {
                        if (currentItemId !== itemId) return;
                ...
                try {
                    ...
                    console.warn('RoNet: Failed to fetch bundle thumbnail', e);
                }
            ...
            console.warn('RoNet: Failed to fetch parent bundle info', e);
        }
    });
}