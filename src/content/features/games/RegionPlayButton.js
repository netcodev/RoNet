...
const NEW_BUTTON_ID = 'ronet-join-preferred-region';
...
const RONET_BUTTON_CLASS = 'ronet-region-button';
...
function injectCustomCSS() {
    const styleId = 'ronet-custom-ui-styles';
    ...
    const css = `
        .${RONET_BUTTON_CLASS} {
            ...
            background-color: var(--ronet-playbutton-color) !important; color: white !important;
            cursor: pointer; border-radius: 12px;
            transition: background-color 0.2s;
            order: 5 !important;
        }
        .${RONET_BUTTON_CLASS}:hover {
            background-color: var(--ronet-playbutton-color) !important;
        }
        .${RONET_BUTTON_CLASS} svg path { 
...
function reconcileButtons(container) {
    ...
    const otherExtensions = children.filter(child => {
        ...
        if (child.id === NEW_BUTTON_ID || child.classList.contains(RONET_BUTTON_CLASS)) return false;
...
function addCustomButton(container) {
    ...
    if (container.querySelector(`.${RONET_BUTTON_CLASS}`) || document.getElementById(NEW_BUTTON_ID)) {
        return false;
    }
    ...
    newButton.className = `btn-primary-md ${RONET_BUTTON_CLASS}`;
