...
function createServerItem(serverData, userThumbnailUrl, userId) {
	...
	serverItem.dataset.ronetServerid = presence.gameId;
...
export function initRecentServers() {
	chrome.storage.local.get({ recentServersEnabled: true }, (settings) => {
		...
		const inject = () => {
			...
			let section = container.querySelector('#rbx-recent-running-games-ronet');
			if (!section) {
				section = document.createElement('div');
				section.id = 'rbx-recent-running-games-ronet';
		...
		observeElement('#rbx-friends-running-games', (friendsSection) => {
			const section = document.getElementById('rbx-recent-running-games-ronet');
			if (section) {
				friendsSection.before(section);
			}
		});

		observeElement('#rbx-recent-running-games-ronet', () => {}, {
			onRemove: () => {
				setTimeout(() => {
					const container = document.querySelector('#roseal-running-game-instances-container') || document.querySelector('#running-game-instances-container');
					if (container && !container.querySelector('#rbx-recent-running-games-ronet')) {
						inject();
					}
				}, 500);
			},
		...
		chrome.runtime.onMessage.addListener((request) => {
			if (request.action === 'presenceUpdate') {
				const section = document.querySelector('#rbx-recent-running-games-ronet');
...
async function renderRecentServers(section) {
	...
	try {
		...
		const result = await new Promise((resolve) => chrome.storage.local.get({ ronet_server_history: {} }, resolve));
		const history = result.ronet_server_history || {};
		...
		activeServers.forEach((serverData) => {
			if (serverData.presence && serverData.presence.gameId) {
				...
				const obsReq = observeElement(`li[data-ronet-serverid="${serverData.presence.gameId}"]`, () => {}, {
					onRemove: () => {
