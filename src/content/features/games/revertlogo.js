...
const HIDE_ROBLOX_UI_CLASS = 'ronet-hide-roblox-dialogs';
...
const styles = `
    ...
    .dark-theme .ronet-info-icon {
        fill: rgb(247, 247, 248) !important;
    }
    .light-theme .ronet-info-icon {
...
function cleanupPolling() {
    ...
    document.removeEventListener('ronet-game-launch-success', handleGameLaunchSuccess);
}
...
async function ensureDatacenterDataIsParsed() {
    try {
        ...
        const result = await Promise.race([
            new Promise((resolve) => chrome.storage.local.get('ronetDatacenters', resolve)),
            new Promise((_, reject) => setTimeout(() => reject(new Error("Timeout")), 2000))
        ]);

        const serverListData = result.ronetDatacenters;
        ...
        console.warn("Ronet: Failed to load datacenter map", error);
    }
}
...
const buildInfoList = (gameId, isPrivateServer, regionCode, regionName, serverInfo, placeVersion, serverChannel, rccVersion, serverUptime, ownerInfo) => {
    const liClass = `class="ronet-details-li"`;

    const verIcon = `<svg class="ronet-info-icon" viewBox="0 0 24 24"><path d="M21 10.12h-6.78l2.74-2.82c-2.73-2.7-7.15-2.8-9.88-.1-2.73 2.71-2.73 7.08 0 9.79s7.15 2.71 9.88 0C18.32 15.65 19 14.08 19 12.1h2c0 1.98-.88 4.55-2.64 6.29-3.51 3.48-9.21 3.48-12.72 0-3.5-3.47-3.53-9.11-.02-12.58s9.14-3.47 12.65 0L21 3zM12.5 8v4.25l3.5 2.08-.72 1.21L11 13V8z"></path></svg>`;
    const timeIcon = `<svg class="ronet-info-icon" viewBox="0 0 24 24"><path d="m22 5.7-4.6-3.9-1.3 1.5 4.6 3.9zM7.9 3.4 6.6 1.9 2 5.7l1.3 1.5zM12.5 8H11v6l4.7 2.9.8-1.2-4-2.4zM12 4c-5 0-9 4-9 9s4 9 9 9 9-4 9-9-4-9-9-9m0 16c-3.9 0-7-3.1-7-7s3.1-7 7-7 7 3.1 7 7-3.1 7-7 7"></path></svg>`;
    const rccIcon = `<svg class="ronet-info-icon" viewBox="0 0 24 24"><path d="M21 3H3c-1.11 0-2 .89-2 2v12c0 1.1.89 2 2 2h5v2h8v-2h5c1.1 0 1.99-.9 1.99-2L23 5c0-1.11-.9-2-2-2m0 14H3V5h18z"></path></svg>`;
    const privIcon = `<svg class="ronet-info-icon" viewBox="0 0 24 24"><path d="M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2m-6 9c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2m3.1-9H8.9V6c0-1.71 1.39-3.1 3.1-3.1s3.1 1.39 3.1 3.1z"></path></svg>`;
    const channelIcon = `<svg class="ronet-info-icon" viewBox="0 0 24 24"><path d="M15 9H9v6h6zm-2 4h-2v-2h2zm8-2V9h-2V7c0-1.1-.9-2-2-2h-2V3h-2v2h-2V3H9v2H7c-1.1 0-2 .9-2 2v2H3v2h2v2H3v2h2v2c0 1.1.9 2 2 2h2v2h2v-2h2v2h2v-2h2c1.1 0 2-.9 2-2v-2h2v-2h-2v-2zm-4 6H7V7h10z"></path></svg>`;

    const listItems = [];

    if (gameId) {
        listItems.push(`<li style="white-space: nowrap;" ${liClass}><strong>ServerID:</strong> <span class="ronet-spoiler" style="white-space: nowrap;">${gameId}</span></li>`);
    }
    ...
    listItems.push(`
        <li ${liClass}>
            ...
            <div class="ronet-channel-wrapper">
                <span class="ronet-channel-truncated">${channelText}</span>
                <span class="ronet-channel-tooltip">${channelText}</span>
            </div>
        </li>
    `);
...
async function pollClientStatus(targetPlaceId) {
    ...
    const showDownloadUI = () => {
        if (document.getElementById('ronet-download-dialog-container')) return;
        ...
        dialogWrapper.id = 'ronet-download-dialog-container';
        ...
        const restartLink = dialogWrapper.querySelector('#ronet-restart-download');
        if (restartLink) {
    ...
    document.addEventListener('ronet-game-launch-success', handleGameLaunchSuccess);
...
const customLogoId = "ronet-custom-logo";
...
function initializeJoinDialogEnhancer() {
    document.addEventListener('ronet-game-servers-response', (event) => {
