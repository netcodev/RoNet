...
const PROCESSED_MARKER_CLASS = 'ronet-quickplay-processed';
const GLOBAL_CONTAINER_ID = 'ronet-private-servers-global-container';
...
function showTemporaryTooltip(parent, text, duration = 1400) {
    ...
    temp.className = 'ronet-ps-tooltip';
...
function initializeData() {
    chrome.storage.local.get(['cachedRegions', 'ronetDatacenters'], (result) => {
        if (result.cachedRegions) State.regions = result.cachedRegions;
        if (result.ronetDatacenters) {
            result.ronetDatacenters.forEach(entry => {
...
function createGlobalPrivateServerContainer() {
    ...
    if (State.privateServersContainer) {
        State.privateServersContainer.className = 'ronet-ps-list-container';
...
async function fetchAndDisplayPrivateServers(placeId, loadMore = false, nextPageCursor = null) {
    ...
    try {
        if (!loadMore) {
            State.privateServersContainer.innerHTML = DOMPurify.sanitize(`<p style="color: var(--ronet-secondary-text-color); text-align: center; padding: 10px 0;">Loading...</p>`);
        ...
        while (true) {
            ...
            if (res.status === 429) {
                if (!loadMore) {
                    State.privateServersContainer.innerHTML = DOMPurify.sanitize(`<p style="color: var(--ronet-secondary-text-color); text-align: center; padding: 10px 0;">Rate limited. Retrying...</p>`);
                }
...
function renderPrivateServers(placeId, servers, nextPageCursor, thumbnails, append) {
    ...
    if (!servers.length && !append) {
        ...
        Object.assign(msg.style, { color: 'var(--ronet-secondary-text-color)', textAlign: 'center', padding: '10px 0' });
...
function initializeQuickPlay() {
    ...
    window.addEventListener('beforeunload', () => {
        ...
        document.getElementById('ronet-global-quickplay-tooltip')?.remove();
