    ...
    if (result.subplacesEnabled) {
        ...
        const checkAndDisplaySubplaceBanner = async (universeId, placeId) => {
            try {
                ...
                console.warn("RoNet: Failed to check for subplace banner", e);
            }
        };
        ...
        const createSubplacesTab = (universeId, horizontalTabs, contentSection) => {
            ...
            searchWrapper.className = 'ronet-subplaces-search-wrapper';
            const searchInputComponent = createStyledInput({
                id: 'ronet-subplaces-search',
            ...
            subplacesContainer.className = 'ronet-subplaces-list';

            const loadMoreWrapper = document.createElement('div');
            loadMoreWrapper.className = 'ronet-load-more-wrapper';
            const loadMoreButton = document.createElement('button');
            loadMoreButton.textContent = 'Load More';
            loadMoreButton.className = 'ronet-load-more-btn btn-control-md';
                ...
                try {
                    ...
                    const displaySubplaces = async (gamesToDisplay) => {
                        ...
                        gamesToDisplay.forEach(subplace => {
                            ...
                            card.classList.add('ronet-subplace-card', 'game-card-container');
                    ...
                    searchInput.addEventListener('input', async () => {
                        ...
                        subplacesContainer.querySelectorAll('.ronet-subplace-card').forEach(c => {
        ...
        const initializeSubplacesFeature = async (tabContainer) => {
            if (tabContainer.dataset.ronetSubplacesInitialized === 'true') {
                return;
            }
            tabContainer.dataset.ronetSubplacesInitialized = 'true';
            ...
            try {
                ...
                tabContainer.dataset.ronetSubplacesInitialized = 'false'; 
            }
        };

        const onTabContainerRemoved = () => {
            const oldTabContainer = document.querySelector('[data-ronet-subplaces-initialized]');
            if (oldTabContainer) {
                oldTabContainer.dataset.ronetSubplacesInitialized = 'false';
            }
        };
