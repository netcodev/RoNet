...
export function init() {
    chrome.storage.local.get({ updateHistoryEnabled: false }, (settings) => {
        ...
        observeElement('#horizontal-tabs', (tabContainer) => {
            if (tabContainer.dataset.ronetUpdatesTabInitialized === 'true') return;
            tabContainer.dataset.ronetUpdatesTabInitialized = 'true';
            ...
            onRemove: () => {
                const oldContainer = document.querySelector('[data-ronet-updates-tab-initialized]');
                if (oldContainer) oldContainer.dataset.ronetUpdatesTabInitialized = 'false';
            }
        });
    });
}
...
async function loadAndRenderHeatmap(placeId, parentElement) {
    ...
    if (universeId) {
        try {
            ...
            console.warn('RoNet: Failed to check content maturity', e);
        }
    }
    ...
    if (!blockingMessage) {
        try {
            ...
            console.warn('RoNet: Failed to check paid access status', e);
        }
    }
    ...
    if (!blockingMessage) {
        try {
            ...
            console.warn('RoNet: Failed to check subplace joinability', e);
        }
    }
    ...
    if (!blockingMessage) {
        try {
            ...
            console.error('RoNet: Failed to load heatmap data', error);
        }
    }
