...
class BotDetector {    
    ...
    async updateBotStats() {
        ...
        const tooltipTextColor = "var(--ronet-secondary-text-color)";
        ...
        if (botPercentage > 20) {
            statsContainer.innerHTML = DOMPurify.sanitize(`
                <div style="display: flex; align-items: center; gap: 4px; color: ${tooltipTextColor};">
                    <span style="font-weight: 500;"><span style="color: var(--ronet-main-text-color)">${gameName}</span> has a lot of bots</span>
            ...
            statsContainer.innerHTML = DOMPurify.sanitize(`
                <div style="display: flex; align-items: center; gap: 4px; color: ${tooltipTextColor};">
                    <span style="font-weight: 500;"><span style="color: var(--ronet-main-text-color);">${gameName}</span> has some bots but mostly real players</span>
                    <i class="icon-moreinfo"></i>
                </div>
            `);
            const infoIcon = statsContainer.querySelector('.icon-moreinfo');
