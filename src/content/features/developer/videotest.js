...
async function loadVideo(assetId, videoElement, statusText, downloadBtn) {
    ...
    statusText.style.color = "var(--ronet-secondary-text-color)";
    ...
    try {
        ...
        if (data && data.length > 0 && data[0].errors) {
            console.warn(`[RoNet] Asset ${assetId} failed as 'Video'. Errors:`, data[0].errors);
...
export async function init() {
    ...
    const isEnabled = await new Promise((resolve) => {
        if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
            ...
            console.warn('[RoNet] Chrome Storage API not found.');
            resolve(false);
        }
    });
    ...
    document.body.style.backgroundColor = 'var(--ronet-container-background-color)';
    document.body.style.color = 'var(--ronet-secondary-text-color)';
    ...
    document.body.innerHTML = `
        <div style="text-align:center;">
            ...
            <video id="ronet-player" controls autoplay style="width: 80vw; max-width: 1000px; box-shadow: 0 0 20px #000; border-radius: 8px; background: #000;"></video>
    ...
    const videoElement = document.getElementById('ronet-player');
