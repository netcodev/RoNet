...
export function init() {
    ...
    chrome.storage.local.get('avatarRotatorEnabled', (data) => {
        ...
        if (!window.location.pathname.includes('/my/avatar')) return;
        if (container.querySelector('.ronet-avatar-rotator-btn')) return;
        ...
        stopBtn.type = 'button';
        stopBtn.className = 'btn-control-xs ronet-avatar-rotator-stop-btn';
        stopBtn.textContent = 'Stop Rotator';
        stopBtn.style.display = 'none';
        stopBtn.onclick = () => {
            chrome.storage.local.set({ ronet_avatar_rotator_enabled: false });
        };

        const rotatorBtn = document.createElement('button');
        rotatorBtn.type = 'button';
        rotatorBtn.className = 'btn-secondary-xs ronet-avatar-rotator-btn';
        ...
        function updateButtonState() {
            ...
            const statusText = document.getElementById('ronet-avatar-status');
            if (statusText) {
        ...
        async function fetchAndRenderAvatars(reset = false) {
            ...
            isLoading = true;
            const loadBtn = document.getElementById('ronet-avatar-load-more');
            ...
            try {
                ...
                if (response.avatarInventoryItems && response.avatarInventoryItems.length > 0) {
                    ...
                    response.avatarInventoryItems.forEach((outfit, index) => {
                        ...
                        card.className = 'ronet-avatar-card';
                        ...
                        const img = createThumbnailElement(thumbData, outfit.itemName, 'ronet-avatar-thumb', { width: '100px', height: '100px', borderRadius: '8px', objectFit: 'cover' });
                ...
                console.error('RoNet: Failed to fetch avatars', error);
            } finally {
                isLoading = false;
                if (loadBtn) {
        ...
        rotatorBtn.addEventListener('click', () => {
            chrome.storage.local.get(['ronet_avatar_rotator_ids', 'ronet_avatar_rotator_enabled', 'ronet_avatar_rotator_interval'], (data) => {
                selectedAvatars.clear();
                if (data.ronet_avatar_rotator_ids && Array.isArray(data.ronet_avatar_rotator_ids)) {
                    data.ronet_avatar_rotator_ids.forEach(id => selectedAvatars.add(id));
                }
                ...
                loadMoreBtn.id = 'ronet-avatar-load-more';
                ...
                placeholderText.id = 'ronet-avatar-status';
                ...
                const { container: inputContainer, input: intervalInput } = createStyledInput({
                    id: 'ronet-rotator-interval',
                    label: 'Interval (seconds)'
                });

                intervalInput.type = 'number';
                intervalInput.min = '5';
                intervalInput.value = data.ronet_avatar_rotator_interval || '5';
                ...
                setRotatorsBtn.onclick = () => {
                    ...
                    chrome.storage.local.set({
                        ronet_avatar_rotator_ids: avatars,
                        ronet_avatar_rotator_enabled: true,
                        ronet_avatar_rotator_interval: interval
                    });
                ...
                disableRotatorBtn.style.display = data.ronet_avatar_rotator_enabled ? 'inline-block' : 'none';
                disableRotatorBtn.onclick = () => {
                    chrome.storage.local.set({
                        ronet_avatar_rotator_enabled: false
                    });
        ...
        function updateStopButtonVisibility() {
            chrome.storage.local.get('ronet_avatar_rotator_enabled', (data) => {
                stopBtn.style.display = data.ronet_avatar_rotator_enabled ? 'inline-block' : 'none';
            });
        }

        updateStopButtonVisibility();
        chrome.storage.onChanged.addListener((changes, namespace) => {
            if (namespace === 'local' && changes.ronet_avatar_rotator_enabled) {
                updateStopButtonVisibility();
            }
        });
    }, { multiple: true });
