...
export function init() {
    const updateState = async (enabled) => {
        document.dispatchEvent(new CustomEvent('ronet-multi-equip', { detail: { enabled } }));
        ...
        if (enabled) {
            try {
                ...
                document.dispatchEvent(new CustomEvent('ronet-multi-equip', {
                    detail: {
                        enabled,
                ...
                console.warn("RoNet: Failed to fetch dynamic categories for multi-equip", e);
            }
        }
    };
