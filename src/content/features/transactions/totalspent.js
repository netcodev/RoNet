...
function onElementFound(container) {
    const buttonIdentifier = 'ronet-total-spent-btn';
                ...
                const itemsHTML = sorted
                    .map((amountStr) => {
                        ...
                        const total = price
                            ...
                            : 'N/A';
                        return `<li>
                                <span class="ronet-breakdown-amount">${formatRobux(amount)} Pack</span>
                                <span class="ronet-breakdown-count">x${count}</span>
                                <span class="ronet-breakdown-price">${total}</span>
                            </li>`;
                    })
                    .join('');
            ...
            if (premiumBreakdownEl) {
                ...
                const itemsHTML = sorted
                    .map((amountStr) => {
                        ...
                        return `<li>
                                <span class="ronet-breakdown-amount">${name}</span>
                                <span class="ronet-breakdown-count">x${count}</span>
                                <span class="ronet-breakdown-price">${total}</span>
                            </li>`;
                    })
            ...
            if (state.calculationType === CALCULATION_TYPE.ROBUX_SPENT) {
                statsGridHTML = `
                    <div class="ronet-stats-grid">
                        ...
                        <div class="ronet-stat-item centered-content">
                            <span class="ronet-stat-label">Total Robux Spent</span>
                            <span class="ronet-stat-value" id="ronet-stat-robux">${robuxSpentValue} <span class="icon-robux-16x16" style="vertical-align: -3px;"></span></span>
                        </div>
                    </div>`;

                breakdownsHTML = `
                    <div class="ronet-breakdown-section">
                        <span class="ronet-stat-label">Item Type Breakdown</span>
                        <div id="ronet-itemtype-breakdown-container"></div>
                    </div>
                `;
            } else {
                statsGridHTML = `
                    <div class="ronet-stats-grid">
                        <div class="ronet-stat-item">
                            <span class="ronet-stat-label">Transactions Scanned</span>
                            <span class="ronet-stat-value" id="ronet-stat-transactions">${transactionsValue}</span>
                        </div>
                        <div class="ronet-stat-item">
                            <span class="ronet-stat-label">Total Spent (Approx.)</span>
                            <span class="ronet-stat-value" id="ronet-stat-money-spent">${moneySpentValue}</span>
                        </div>
                    </div>`;
                breakdownsHTML = `
                    <div class="ronet-breakdown-section">
                        <span class="ronet-stat-label">Premium Subscription Breakdown</span>
                        <div id="ronet-premium-breakdown-container"></div>
                    </div>
                    <div class="ronet-breakdown-section">
                        <span class="ronet-stat-label">Robux Purchase Breakdown</span>
                        <div id="ronet-purchase-breakdown-container"></div>
                    </div>
                `;
            }
    ...
    const runCalculation = async () => {
        ...
        try {
            ...
            if (error instanceof PausedException) {
                console.log(`RoNet: ${error.message}`);
                await animationController.waitUntilIdle();
            } else {
                console.error(
                    'RoNet: An error occurred during calculation:',
                    error,
                );
