...
function injectResultElement(targetElement, result) {
    ...
    if (result.isLoading) {
        amountHtml = `<span style="color: var(--ronet-main-text-color); font-weight: 400; font-size: 13px;">Loading...</span>`;
        ...
        amountHtml = `<span style="color: var(--ronet-main-text-color); font-weight: 400; font-size: 13px;">Insufficient data</span>`;
        tooltipText =
            'Not enough transaction history to make an accurate estimate. Please wait for more transactions to complete.';
    } else {
        amountHtml = `
            <span class="icon-robux-16x16"></span>
            <span class="text-robux" style="color: var(--ronet-main-text-color); font-weight: 400;">${result.amount.toLocaleString()}~</span>
        `;
    }
    ...
    if (estimatorRow) {
        ...
        estimatorRow.innerHTML = `
            <td class="unpending-sales" style="display: flex; align-items: center;">
                <div style="color: var(--ronet-main-text-color);">
                    <span class="ng-binding">Unpending Robux tomorrow</span>
                </div>
                <span class="icon-moreinfo" style="margin-left: 4px; font-size: 12px; display: inline-flex; align-items: center; color: var(--ronet-main-text-color); cursor: pointer;"></span>
            </td>
...
export function init() {
    chrome.storage.local.get({ pendingrobuxtrans: true }, async (settings) => {
        ...
        try {
            ...
            if (!state.userId) {
                console.error(
                    'RoNet: Could not get user ID for pending Robux feature.',
                );
                return;
            }
            ...
            console.error(
                'RoNet: Failed to initialize pending Robux feature.',
                e,
            );
        }
    });
