...
function addPriceIconToCard(card, assetId) {
    ...
    if (isOffSale && price !== undefined && price > 1) {
        ...
        let container;
        const priceLabelSelector = '.text-overflow.item-card-price, .ronet-item-rap';
        ...
        if (container && !container.querySelector('.ronet-offsale-price-icon')) {
...
export function init() {
    chrome.storage.local.get('PreviousPriceEnabled', (result) => {
        ...
        window.addEventListener('ronet-catalog-details', async (e) => {
            const data = e.detail;
        ...
        observeElement('.ronet-item-card', (card) => {
            handleItemCard(card);
        }, { multiple: true });
...
function handleItemCard(card) {
    ...
    const link = card.querySelector('.item-card-link') || card.querySelector('.ronet-item-card-link');
    if (!link) return;
    ...
    if (shouldProcess) {
        ...
        if (!card.querySelector('.ronet-offsale-price-icon')) {
            if (!pendingCards.has(assetId)) {
...
function handleOffsalePriceContainer(container) {
    if (container.dataset.ronetPreviousPrice) return;
    container.dataset.ronetPreviousPrice = 'true';
...
function addIcon(container, price) {
    if (container.querySelector('.ronet-offsale-price-icon')) return;
    
    const assets = getAssets();
    const icon = document.createElement('div');
    icon.className = 'ronet-offsale-price-icon';
...
function addTextPrice(container, price) {
    if (container.querySelector('.ronet-previous-price-text')) return;

    const div = document.createElement('div');
    div.className = 'ronet-previous-price-text';
    div.style.marginTop = '5px';
    div.style.fontSize = '12px';
    div.style.color = 'var(--ronet-secondary-text-color)';
