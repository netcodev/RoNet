...
const STORAGE_KEY = 'ronet_search_history';
...
async function performUserSearch(query) {
    ...
    try {
        ...
        const localFriendsPromise = new Promise(resolve => {
            chrome.storage.local.get(['ronet_friends_data'], (result) => {
                if (!searchSettings.friendSearchEnabled) {
                    resolve([]);
                    return;
                }

                const friendsData = result.ronet_friends_data?.[authedUserId];
        ...
        localFriendsPromise.then(async (localFriends) => {
            ...
            if (localFriends.length > 0) {
                ...
                window._lastRoNetFriendResults = validUsers.map(friendUser => {
                    if (window._lastRoNetUserResult && window._lastRoNetUserResult.dataset.userId == friendUser.id) {
                        return null;
                    }
                ...
                window._lastRoNetFriendResults = [];
            }
            injectIntoMenu();
        }).catch(e => {
            if (e.name !== 'AbortError') console.error('RoNet: Friend Search Error', e);
        });
        ...
        if (userResult) {
            ...
            window._lastRoNetUserResult = createUserResultHtml(userDetails, userThumbData, userPresence, userResultIsFriend, isTrusted);
            
            if (window._lastRoNetFriendResults) {
                window._lastRoNetFriendResults = window._lastRoNetFriendResults.filter(el => el.dataset.userId != userResult.id);
            }
        } else {
            window._lastRoNetUserResult = null;
        }

        injectIntoMenu();
    } catch (e) {
        if (e.name !== 'AbortError') console.error('RoNet: User Search Error', e);
    }
}
...
async function performGameSearch(query) {
    ...
    try {
        ...
        if (friendsPlaying.length > 0 && results[2]) {
            ...
            window._lastRoNetGameResult = createResultHtml(game, thumbnailUrl, playerCount, voteRatio, totalVotes, settings, friendsInfo);
        }

        injectIntoMenu();
    } catch (e) {
        if (e.name !== 'AbortError') console.error('RoNet: Game Search Error', e);
    }
}

function createUserResultHtml(user, thumbData, presence, isFriend = false, isTrusted = false) {
    const li = document.createElement('li');
    li.className = 'navbar-search-option rbx-clickable-li improved-search ronet-quick-search-result';
    ...
    if (presence) {
        ...
        if (presenceClass) {
            ...
            Object.assign(presenceIndicator.style, {
                ...
                borderRadius: '50%',
                border: '2px solid var(--ronet-container-background-color)'
            });
    ...
    Object.assign(displayNameDiv.style, {
        fontSize: '16px',
        fontWeight: '500',
        color: 'var(--ronet-main-text-color)',
        whiteSpace: 'nowrap',
        overflow: 'hidden',
    ...
    Object.assign(secondaryInfoDiv.style, {
        fontSize: '12px',
        color: 'var(--ronet-secondary-text-color)',
        whiteSpace: 'nowrap',
        overflow: 'hidden',
        textOverflow: 'ellipsis'
    });
    ...
    if (user.isBanned) {
        if (displayNameDiv) {
            ...
            Object.assign(lockIcon.style, {
                ...
                marginLeft: '5px',
                flexShrink: '0',
                backgroundColor: 'var(--ronet-secondary-text-color)',
    ...
    if (presence && presence.userPresenceType === 2 && presence.gameId) {
        ...
        Object.assign(playBtn.style, {
            backgroundColor: 'var(--ronet-playbutton-color)',
            border: 'none',
            borderRadius: '8px',
            width: '36px',
            height: '36px',
...
function createResultHtml(game, thumbnailUrl, playerCount, voteRatio, totalVotes, settings, friendsInfo) {
    ...
    link.innerHTML = safeHtml`
        ...
        <div style="display: flex; flex-direction: column; justify-content: center; overflow: hidden; width: 100%;">
            <div class="game-card-name" title="${game.name}" style="font-size: 16px; font-weight: 500; color: var(--ronet-main-text-color); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${game.name}</div>
            <div class="game-card-info" style="display: flex; align-items: center; gap: 4px; margin-top: 4px; font-size: 12px; color: var(--ronet-secondary-text-color);">
...
function injectIntoMenu() {
    ...
    menu.querySelectorAll('.ronet-quick-search-result').forEach(el => el.remove());
    
    if (window._lastRoNetGameResult) {
        menu.prepend(window._lastRoNetGameResult);
    }
    if (window._lastRoNetFriendResults && window._lastRoNetFriendResults.length > 0) {
        window._lastRoNetFriendResults.slice().reverse().forEach(friendResult => {
            menu.prepend(friendResult);
        });
    }
    if (window._lastRoNetUserResult) {
        menu.prepend(window._lastRoNetUserResult);
    }

    const firstResult = menu.querySelector('.ronet-quick-search-result');
    const currentlySelected = menu.querySelector('.new-selected');
    if (firstResult && currentlySelected && !currentlySelected.classList.contains('ronet-quick-search-result')) {
...
function injectExistingResult() {
    const menu = document.querySelector('ul.new-dropdown-menu');
    if (menu && !menu.querySelector('.ronet-quick-search-result')) {
        if (window._lastRoNetGameResult) {
            menu.prepend(window._lastRoNetGameResult);
        }
        if (window._lastRoNetFriendResults && window._lastRoNetFriendResults.length > 0) {
...
function handleButtonState(btn, shouldDisable) {
    const currentlyDisabled = btn.classList.contains('ronet-btn-disabled');
    
    if (shouldDisable) {
        if (!currentlyDisabled) {
            btn.classList.add('ronet-btn-disabled');
            // Double click prevention
            btn.style.pointerEvents = 'auto';
            
            if (btn._timeout) clearTimeout(btn._timeout);
            btn._timeout = setTimeout(() => {
                if (btn.classList.contains('ronet-btn-disabled')) {
        ...
        if (currentlyDisabled) {
            btn.classList.remove('ronet-btn-disabled');
...
async function renderSearchHistory(container) {
    if (container.querySelector('.ronet-search-history-section')) {
        container.querySelector('.ronet-search-history-section').remove();
    }
    const history = await getHistory();

    const section = document.createElement('div');
    section.className = 'game-sort-header-container ronet-search-history-section';
    ...
    scrollWrapper.className = 'ronet-history-scroll-wrapper';
    scrollWrapper.id = 'ronet-history-list-scroll';
    scrollWrapper.style.position = 'relative';
    scrollWrapper.style.padding = '10px 0';

    const listContainer = document.createElement('div');
    listContainer.className = 'ronet-history-list-container';
    ...
    if (!document.getElementById('ronet-history-scrollbar-style')) {
        const style = document.createElement('style');//verified
        style.id = 'ronet-history-scrollbar-style';
        style.textContent = `
            #ronet-history-list-scroll .ronet-history-list-container::-webkit-scrollbar { display: none; }
            #ronet-history-list-scroll .ronet-scroll-btn {
                position: absolute;
                top: 50%;
            ...
            #ronet-history-list-scroll .ronet-scroll-btn:hover {
                background-color: rgba(0, 0, 0, 0.8);
            }
            #ronet-history-list-scroll .ronet-scroll-btn.left { left: 0; }
            #ronet-history-list-scroll .ronet-scroll-btn.right { right: 0; }
            #ronet-history-list-scroll .ronet-scroll-btn.ronet-btn-disabled {
                opacity: 0;
                cursor: default;
                pointer-events: none;
            }
    ...
    list.className = 'ronet-history-list';
    ...
    const { leftButton, rightButton } = createScrollButtons({
        onLeftClick: (e) => {
            if (!e.currentTarget.classList.contains('ronet-btn-disabled')) {
    ...
    leftButton.classList.add('ronet-scroll-btn', 'left');
    rightButton.classList.add('ronet-scroll-btn', 'right');
    ...
    if (container._ronetResizeCleanup) {
        container._ronetResizeCleanup();
    }
    ...
    container._ronetResizeCleanup = unobserve;
    ...
    const seeker = setInterval(() => {
        ...
        if (input) {
            ...
            input.addEventListener('input', () => {
                ...
                window._lastRoNetUserResult = null;
                window._lastRoNetGameResult = null;
                window._lastRoNetFriendResults = [];
                injectIntoMenu();
            ...
            observeElement('section[data-testid="SearchLandingPageOmniFeedTestId"]', (container) => {
                ...
                onRemove: (container) => {
                    if (container._ronetResizeCleanup) {
                        container._ronetResizeCleanup();
                        delete container._ronetResizeCleanup;
                    }
                }
            });
}
