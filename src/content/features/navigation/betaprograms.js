...
export async function addNavbarButton() {
    if (document.getElementById('ronet-beta-programs-toggle')) return;

    const assets = getAssets();
    const icon = assets.TerminalIcon;

    const button = await createNavbarButton({
        id: 'ronet-beta-programs-toggle',
        iconSvgData: icon,
    ...
    button.addEventListener('click', async (e) => {
        ...
        try {
            ...
            if (menuItems.length === 0) {
                ...
                noProgramsEl.className = 'ronet-dropdown-item';
                ...
                menuItems.forEach(item => {
                    ...
                    itemEl.className = 'ronet-dropdown-item flex items-center justify-between p-2';
            ...
            console.error('RoNet: Failed to fetch beta programs', error);
