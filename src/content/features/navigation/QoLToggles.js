...
export function init() {
    chrome.storage.local.get({ qolTogglesEnabled: true }, (settings) => {
        ...
        const addQoLButton = async () => {
            if (document.getElementById('ronet-qol-toggle')) return;
            ...
            if (!button) return;
                ...
                try {
                    ...
                    console.warn('RoNet: Failed to fetch online status', e);
                }
                ...
                chrome.storage.local.get([], (data) => {
                    ...
                    const itemButtons = menu.panel.querySelectorAll('.ronet-dropdown-item');
                        ...
                        if (value === 'onlineStatus' || value === 'joinStatus') {
                            ...
                            const { element: statusDropdown, setValue } = createDropdown({
                                ...
                                onValueChange: (newValue) => {
                                    ...
                                    const onlineDropdownEl = document.getElementById('ronet-qol-onlineStatus-dropdown');
                                    const joinDropdownEl = document.getElementById('ronet-qol-joinStatus-dropdown');
                                    ...
                                    if (isOnlineStatus) {
                                        ...
                                        if (onlineLevel < joinLevel) {
                                            ...
                                            if (joinDropdownEl && joinDropdownEl.ronetSetValue) {
                                                joinDropdownEl.ronetSetValue(newJoinValue);
                                        ...
                                        if (joinLevel > onlineLevel) {
                                            ...
                                            if (onlineDropdownEl && onlineDropdownEl.ronetSetValue) {
                                                onlineDropdownEl.ronetSetValue(newOnlineValue);
                                    ...
                                    if (isOnlineStatus && newValue === 'NoOne' && joinDropdownEl && joinDropdownEl.ronetSetValue) {
                                        const joinDropdownEl = document.getElementById('ronet-qol-joinStatus-dropdown');
                                        if (joinDropdownEl && joinDropdownEl.ronetSetValue) {
                                            joinDropdownEl.ronetSetValue('NoOne');
                                            callRobloxApi({
                                                subdomain: 'apis',
                                                endpoint: '/user-settings-api/v1/user-settings',
                                                method: 'POST',
                            ...
                            statusDropdown.id = `ronet-qol-${value}-dropdown`;
                            statusDropdown.ronetSetValue = setValue;
                            ...
                            const trigger = statusDropdown.querySelector('.ronet-dropdown-trigger');
                            if (trigger) {
