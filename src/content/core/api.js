...
const OAUTH_STORAGE_KEY = 'ronet_oauth_verification';
const CAPTURED_APIS_KEY = 'ronet_captured_apis';
const seenRequests = new Map();

document.addEventListener('ronet-traffic-capture', (e) => {
    ...
    try {
        ...
        } else if (hostname === 'rovalra.com' || hostname.endsWith('.rovalra.com')) {
            isRonetApi = true;
            subdomain = hostname === 'rovalra.com' ? 'www' : hostname.replace('.rovalra.com', '');
        } else {
            return;
        }
...
function captureApiCall(options) {
    try {
        const { subdomain = 'apis', endpoint, method = 'GET', isRonetApi = false, body = null } = options;
        if (!endpoint) return;
        
        const [baseEndpoint] = endpoint.split('?');
        const category = isRonetApi ? 'rovalra.com' : (subdomain || 'apis');
...
function getRequestKey({ endpoint, subdomain = 'apis', method = 'GET', isRonetApi = false, body = null, fullUrl = null }) {
    const bodyStr = body && typeof body === 'object' ? JSON.stringify(body) : (body || '');
    const target = fullUrl || `${isRonetApi}|${subdomain}|${endpoint}`;
...
function checkSimulatedDowntime() {
    return new Promise((resolve) => {
        ...
        chrome.storage.local.get(['simulateRoNetServerErrors'], (result) => {
            resolve(!!result.simulateRoNetServerErrors);
        });
    });
}
...
function checkSimulatedLatency() {
    return new Promise((resolve) => {
        ...
        chrome.storage.local.get(['simulateRoNetServerLatency'], (result) => {
            resolve(!!result.simulateRoNetServerLatency);
        });
    });
}
...
export async function callRobloxApi(options) {
    ...
    if (shouldCache && activeRequests.has(requestKey)) {
        ...
        if (options.subdomain === 'games' && options.endpoint.includes('/servers/') && !options.isRonetApi) {
        }
    ...
    const requestPromise = (async () => {
        ...
        if (isRonetApi) {
            ...
            if (isDowntimeSimulated) {
                console.warn(`RoNet API: [SIMULATION] 500 Error for ${endpoint}`);
            ...
            if (isLatencySimulated) {
                console.warn(`RoNet API: [SIMULATION] Adding 5s latency for ${endpoint}`);
                await new Promise(resolve => setTimeout(resolve, 5000));
            }
        }

        const baseUrl = isRonetApi 
        ...
        const credentials = options.credentials ?? (isRonetApi ? 'omit' : 'include');
...
// TODO This dont work update it at some point.
                        ...
                        try {
                            ...
                            if (authedUserId) {
                                ...
                                if (storedVerification) {
                                    console.log("RoNet API: New token received from header. Updating storage.");
                            ...
                            console.error("RoNet API: Failed to update new access token.", e);
                        }
                    }

                
                    if (lastResponse.status === 401 && endpoint && endpoint.includes('/v1/auth') && !skipAutoAuth && !authRetried) {
                        console.log("RoNet API: 401 Unauthorized, attempting token refresh...");
                        authRetried = true;
                    ...
                    if (attempt === 3 || (endpoint && endpoint.includes('/v1/auth'))) {
                        console.error(`RoNet API: Request to ${fullUrl} failed${attempt === 3 ? ' after multiple retries' : ''}.`, error);
                        throw error;
                    }
                }
                if (attempt < 3) {
            ...
            if (!lastResponse.ok) {
                console.error(`RoNet API: Request to ${fullUrl} failed with status ${lastResponse.status} after multiple retries.`);
            }
            return lastResponse;
        }
        ...
        if (!response.ok) {
            console.error(`RoNet API: Request to ${fullUrl} failed with status ${response.status}.`);
        }
    ...
    if (options.subdomain === 'gamejoin' && originalResponse.ok) {
        ...
        gameJoinClone.json().then(data => {
            ...
            if (data.status === 5) {
                ...
                if (serverId) {
                    document.dispatchEvent(new CustomEvent('ronet-server-inactive', {
                        detail: { serverId }
                    }));
                }
            }
    ...
    if (options.subdomain === 'games' && options.endpoint.includes('/servers/') && !options.isRonetApi) {
        try {
            ...
            monitorClone.json().then(data => {
                document.dispatchEvent(new CustomEvent('ronet-game-servers-response', {
                    detail: { url: fullUrl, data: data }
                }));
            }).catch(() => {});
        } catch (e) {
            console.warn('RoNet API: Monitor hook failed', e);
        }
    }
