...
export const dataPromise = new Promise((resolve, reject) => {
    chrome.storage.local.get(['ronetDatacenters'], (result) => {
        REGIONS = {};
        serverIpMap = {};

        if (result.ronetDatacenters && Array.isArray(result.ronetDatacenters)) {
            for (const entry of result.ronetDatacenters) {
...
export async function findClosestServerViaApi(placeId, originRegionId, userRequestedStop) {
    ...
    for (const { region } of regionsToCheck) {
        ...
        try {
            const response = await callRobloxApi({ isRonetApi: true, endpoint: url });
            if (response.ok) {
...
export async function findServerViaRonetApi(placeId, universeId, preferredRegionId, failedRegionNames, joinedServerIds, userRequestedStopCheck) {
    try {
        ...
        console.error("Ronet Search Error", error);
        return { status: 'API_ERROR' };
    }
}
...
async function getRankedRegions(placeId, preferredRegionId) {
    ...
    if (FINDER_CONFIG.logScores) {
        console.log(`[RoNet] Region Scores for Place ${placeId}:`);
        ranked.forEach(r => {
