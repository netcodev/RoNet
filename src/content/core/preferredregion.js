...
export async function performJoinAction(placeId, universeId, preferredRegionCode = null, onCancel = null) {
    ...
    try {
        ...
        if (!userRequestedStop) {
            ...
            const ronetResult = await ClosestServer.findServerViaRonetApi(
                placeId, 
                universeId, 
                preferredRegionCode, 
                failedRegionNames, 
            ...
            if (ronetResult.status === 'JOINED') {
                joined = true;
                runManualScan = false;
            } else if (ronetResult.status === 'NO_SERVERS') {
                runManualScan = true;
...
export async function getSavedPreferredRegion() {
    ...
    if (region && region !== 'AUTO' && !REGIONS[region]) {
        try {
            ...
            console.error("RoNet: Failed to reset invalid preferred region.", e);
        }
        return 'AUTO';
    }
