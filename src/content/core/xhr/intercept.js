(function() {
    ...
    if (window.__RONET_INTERCEPTOR_SETUP__) {
        return;
    }
    window.__RONET_INTERCEPTOR_SETUP__ = true;
    ...
    function dispatchCaptureEvent(url, method, body) {
        ...
        document.dispatchEvent(new CustomEvent('ronet-traffic-capture', {
    ...
    try {
        streamerModeEnabled = sessionStorage.getItem('ronet_streamermode') === 'true';
        settingsPageInfoEnabled = sessionStorage.getItem('ronet_settingsPageInfo') !== 'false';
    } catch (e) {}

    document.addEventListener('ronet-streamer-mode', (e) => {
    ...
    window.fetch = async function(...args) {
            ...
            if (isSensitive) {
                try {
                    ...
                    if (url.includes('/my/settings/json')) {
                        data.UserEmail = "RoNet Streamer Mode Enabled";
                        data.UserEmailVerified = true;
                    }
                    if (url.includes('v1/phone')) { 
                        data.phone = data.prefix = data.countryCode = "RoNet Streamer Mode Enabled"; 
                    }
                    ...
                    if (url.includes('account-country') && data.value) {
                        data.value.countryName = data.value.localizedName = "RoNet Streamer Mode Enabled";
                        data.value.countryId = 1;
                    }
                    if (url.includes('age-group')) {
                        data.ageGroupTranslationKey = "RoNet Streamer Mode Enabled";
                    }
                    if (url.includes('sessions') && data.sessions) {
                        data.sessions.forEach(s => { 
                            if(s.location) {
                                ...
                                s.location.country = "To view your sessions please disable \"RoNet streamer mode\"";
                            }
                            if(s.agent) {
                                s.agent.os = "RoNet streamer mode enabled";
                                s.agent.type = "App";
                            }
        ...
        if (typeof url === 'string') {
            if (url.includes(CATALOG_API_URL)) {
                response.clone().json().then(d => window.dispatchEvent(new CustomEvent('ronet-catalog-details', { detail: d }))).catch(()=>{});
            }
            if (url.includes(CATALOG_API_URL)) {
                response.clone().json().then(d => document.dispatchEvent(new CustomEvent('ronet-catalog-details-response', { detail: d }))).catch(()=>{});
            }
            if (url.includes(CLIENT_STATUS_API_URL)) {
                response.clone().json().then(d => document.dispatchEvent(new CustomEvent('ronet-client-status-response', { detail: d }))).catch(()=>{});
            }
            if (url.includes(GAME_LAUNCH_SUCCESS_URL) && (url.includes('GameLaunchSuccessWeb_Win32'))) {
                document.dispatchEvent(new CustomEvent('ronet-game-launch-success', { detail: { url } }));
            }
            if (url.includes(GAME_SERVERS_API_URL) && url.includes('/servers/')) {
                response.clone().json().then(d => document.dispatchEvent(new CustomEvent('ronet-game-servers-response', { detail: { url, data: d } }))).catch(()=>{});
            }
            if (url.includes(GAMES_ROBLOX_API) && url.includes('/media')) {
                response.clone().json().then(d => document.dispatchEvent(new CustomEvent('ronet-game-media-response', { detail: d }))).catch(()=>{});
            }
        }
    ...
    XMLHttpRequest.prototype.open = function(method, url, ...rest) {
        this._ronet_url = url; 
        this._ronet_method = method;

        if (streamerModeEnabled && typeof url === 'string' && settingsPageInfoEnabled) {
            if (url.includes('/my/settings/json')) this._ronet_spoof_settings = true;
            if (url.includes('/v1/emails')) this._ronet_email_settings = true;
            if (url.includes('v1/phone')) this._ronet_spoof_phone = true;
            if (url.includes('v1/birthdate')) this._ronet_spoof_birthdate = true;
            if (url.includes('verified-age')) this._ronet_spoof_age = true;
            if (url.includes('account-country')) this._ronet_spoof_country = true;
            if (url.includes('age-group')) this._ronet_spoof_age_group = true;
            if (url.includes('sessions')) this._ronet_spoof_sessions = true;
        }
    ...
    XMLHttpRequest.prototype.send = function(...args) {
        const xhr = this;
        try { dispatchCaptureEvent(xhr._ronet_url, xhr._ronet_method, args[0]); } catch (e) {}

        if (xhr._ronet_spoof_settings || xhr._ronet_spoof_phone || xhr._ronet_spoof_birthdate || xhr._ronet_spoof_age || xhr._ronet_spoof_country || xhr._ronet_spoof_age_group || xhr._ronet_spoof_sessions) {
            ...
            Object.defineProperty(xhr, 'responseText', {
                ...
                get: function() {
                    ...
                    try {
                        ...
                        if (xhr._ronet_spoof_settings) { data.UserEmail = "RoNet Streamer Mode Enabled"; data.UserEmailVerified = true; data.PreviousUserNames = "RoNet Streamer Mode Enabled"; data.UserEmailMasked = false; }
                        if (xhr._ronet_email_settings) { data.verifiedEmail = "RoNet Streamer Mode Enabled"; }
                        if (xhr._ronet_spoof_phone) { data.countryCode = data.prefix = data.phone = "RoNet Streamer Mode Enabled"; }
                        if (xhr._ronet_spoof_birthdate) { data.birthMonth = data.birthDay = data.birthYear = 0; }
                        if (xhr._ronet_spoof_age) { data.isVerified = true; data.verifiedAge = 0; data.isSeventeenPlus = false; }
                        if (xhr._ronet_spoof_country && data.value) { data.value.countryName = data.value.localizedName = "RoNet Streamer Mode Enabled"; data.value.countryId = 1; }
                        if (xhr._ronet_spoof_age_group) { data.ageGroupTranslationKey = "RoNet Streamer Mode Enabled"; }
                        if (xhr._ronet_spoof_sessions && data.sessions) {
                            data.sessions.forEach(s => {
                                if (s.location) {
                                    s.location.city = s.location.subdivision = "";
                                    s.location.country = "To view your sessions please disable \"RoNet streamer mode\"";
        ...
        xhr.addEventListener('load', function() {
            if (typeof xhr._ronet_url === 'string') {
                const triggerEvent = (eventName, detail) => document.dispatchEvent(new CustomEvent(eventName, { detail }));
                try {
                    const url = xhr._ronet_url;
                    if (url.includes(CATALOG_API_URL)) window.dispatchEvent(new CustomEvent('ronet-catalog-details', { detail: JSON.parse(xhr.responseText) }));
                    if (url.includes(CATALOG_API_URL)) triggerEvent('ronet-catalog-details-response', JSON.parse(xhr.responseText));
                    if (url.includes(CLIENT_STATUS_API_URL)) triggerEvent('ronet-client-status-response', JSON.parse(xhr.responseText));
                    if (url.includes(GAME_SERVERS_API_URL) && url.includes('/servers/')) triggerEvent('ronet-game-servers-response', { url, data: JSON.parse(xhr.responseText) });
                    if (url.includes(GAMES_ROBLOX_API) && url.includes('/media')) triggerEvent('ronet-game-media-response', JSON.parse(xhr.responseText));
                } catch (e) {}
            }
        });
    ...
    document.addEventListener('ronet-multi-equip', (e) => {
        if (e.detail) {
    ...
    const patchAvatarService = (service) => {
        if (!service || service.__ronet_patched) return;
        service.__ronet_patched = true;
        ...
        console.log("RoNet: Multi-Accessory patch applied.");
    };
    ...
    console.log('RoNet: Request capture, Privacy Spoofing, and Multi-Accessory loaded successfully.');
})();