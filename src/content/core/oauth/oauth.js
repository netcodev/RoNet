...
const STORAGE_KEY = "ronet_oauth_verification";
...
export async function init() {
    try {
        console.log("RoNet: Script loaded. Syncing session...");
        

        const token = await getValidAccessToken(true);

        if (token) {
            console.log("RoNet: Session synchronized successfully.");
        } else {
            console.log("RoNet: No active session or re-auth required.");
        }
    } catch (error) {
        console.error("RoNet: Error during script initialization", error);
    }
}
export async function getValidAccessToken(forceRefresh = false) {
    ...
    try {
        ...
        if (!response.ok) {
            console.warn(`RoNet: Session invalid (Status ${response.status}). Triggering re-auth...`);
            const success = await startOAuthFlow(true);
            if (success) {
        ...
        console.error("RoNet: Network error during token sync:", error);
        return storedVerification.accessToken; 
    }
}
...
async function checkUserExistence(userId) {
    ...
    try {
        ...
        console.error("RoNet: Error checking user existence", error);
    }
    return false;
}
...
async function startOAuthFlow(silent = false) {
    ...
    if (!silent) {
        console.warn("RoNet: Non-silent OAuth flow is not implemented as per the background-only request.");
    ...
    activeOAuthPromise = (async () => {
        try {
            ...
            if (birthResponse.ok) {
                ...
                if (age < 13) {
                    console.log("RoNet: User is under 13. Skipping OAuth.");
                    return false;
                }
            }
        } catch (error) {
            console.warn("RoNet: Failed to check birthdate", error);
        }
        ...
        try {
            console.log("RoNet: Attempting direct OAuth authorization POST request...");
            ...
            if (response.ok) {
                ...
                if (!locationUrl) {
                    console.error("RoNet: OAuth authorization response did not contain a location URL.", authResponse);
                    return false;
                }

                console.log("RoNet: Got authorization code. Fetching token from callback URL...");
                ...
                if (!tokenResponse.ok) {
                    console.error("RoNet: Failed to get token from callback URL.", await tokenResponse.text());
                    return false;
                }
                ...
                if (tokenData.status === 'success' && tokenData.access_token && tokenData.user_id && tokenData.username) {
                    console.log("RoNet: OAuth Successful!", tokenData);
                    ...
                    console.error("RoNet: Invalid token data received from backend.", tokenData);
                    return false;
                }
            } else {
                console.error("RoNet: OAuth authorization POST request failed with status " + response.status, await response.text());
                return false;
            }

        } catch (error) {
            console.error("RoNet: Error during direct OAuth authorization request.", error);
            return false;
        }
    })();
