...
const REVIEW_URL = "https://chromewebstore.google.com/detail/ronet-roblox-improved/njcickgebhnpgmoodjdgohkclfplejli/reviews";
const STATUS_KEY = "ronet_review_popup_status";
const STATS_KEY = "ronet_review_stats";
...
export function showReviewPopup(source = 'unknown') {
    observeElement('body', () => {
        chrome.storage.local.get([STATUS_KEY, STATS_KEY, 'forceReviewPopup'], (result) => {
            ...
            setTimeout(() => {
                if (document.querySelector('.ronet-global-overlay')) {
                    let obsRequest;
                    ...
                    obsRequest = observeElement('.ronet-global-overlay', () => {}, { onRemove: onOverlayRemoved });
                    return;
                }
                if (document.querySelector('.ronet-review-popup-marker')) return;

                const bodyContent = document.createElement('div');
                bodyContent.className = 'ronet-review-popup-marker';
                bodyContent.style.display = 'flex';
                bodyContent.style.flexDirection = 'column';
                bodyContent.style.gap = '10px';

                const text = document.createElement('p');
                text.textContent = "It looks like you've been getting value from RoNet!\n Would you mind leaving a review?\n It helps keep RoNet free for everyone without paywalls.";
