...
async function fetchLatestPlaceVersion(placeId) {
    ...
    try {
        ...
        console.error('RoNet Server Stats: Failed to fetch latest place version from Roblox.', error);
        return null;
    }
}
...
export async function fetchServerStats(placeId) {
    ...
    try {
        ...
        console.error('RoNet Server Stats: Failed to fetch server statistics.', error);
        return null;
    }
}
...
async function createStatsBarUI(serverListContainer) {
    ...
    try {
        ...
        console.error('RoNet Server Stats: Failed to fetch settings.', error);
        return;
    }
    ...
    serverListContainer.querySelector('.ronet-stats-container')?.remove();
    ...
    statsContainer.className = 'ronet-stats-container';
    ...
    if (hasActiveStats) {
        ...
        totalServersBar.className = `ronet-region-stats-bar ${theme}`;
        ...
        addTooltip(totalServersBar, 'Total servers RoNet is tracking under this experience', { position: 'top' });
    ...
    if (settings.GameVersionEnabled === true && counts.newest_place_version) {
        ...
        versionBar.className = `ronet-region-stats-bar ${theme}`;
    ...
    if (settings.OldestVersionEnabled === true && counts.oldest_place_version && hasActiveStats) {
        ...
        oldestVersionBar.className = `ronet-region-stats-bar ${theme}`;
    ...
    if (serverListContainer.closest('#roseal-running-game-instances-container')) {
        const controls = document.getElementById('ronet-main-controls');
...
export async function initGlobalStatsBar() {
    ...
    try {
        ...
        console.error('RoNet Server Stats: Failed to initialize stats bar.', error);
    }
}
