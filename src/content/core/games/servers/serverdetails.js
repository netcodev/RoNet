...
const CLASSES = {
    CONTAINER: 'ronet-details-container',
    INFO_ROW: 'text-info',
    Region: 'ronet-region-info',
    Uptime: 'ronet-uptime-info',
    Performance: 'ronet-performance-info',
    Version: 'ronet-version-info',
    Full: 'ronet-server-full-info',
    Private: 'ronet-private-server-info',
    Purchase: 'ronet-purchase-game-info',
    Inactive: 'ronet-inactive-place-info'
};
...
let ronetDatacentersCache = null;
...
const cacheReadyPromise = new Promise(resolve => {
    ...
    chrome.storage.local.get([
        ...
        if (res?.ronetDatacenters) ronetDatacentersCache = res.ronetDatacenters;
    ...
    chrome.storage.onChanged?.addListener((changes, area) => {
        if (area === 'local') {
            if (changes.ronetDatacenters) ronetDatacentersCache = changes.ronetDatacenters.newValue;
            if (changes.ServerlistmodificationsEnabled) {
                ...
                if (!isServerListModificationsEnabled) {
                    document.querySelectorAll('.ronet-details-container, .ronet-server-extra-details, .ronet-copy-join-link').forEach(el => el.remove());
                } else {
                }
            }
            ...
            if (changes.EnableDatacenterandId) {
                ...
                document.querySelectorAll('[data-ronet-serverid]').forEach(server => {
...
function getLocationFromDataCenterId(id) {
    if (!ronetDatacentersCache || !id) return null;
    const numId = Number(id);
    if (isNaN(numId)) return null;

    return ronetDatacentersCache.find(entry => 
...
function extractCountryCode(regionName) {
    if (!regionName || !ronetDatacentersCache?.length) return null;
    ...
    for (const entry of ronetDatacentersCache) {
...
function removeCountryFromRegion(regionName) {
    if (!regionName || !ronetDatacentersCache) return regionName;
    
    const parts = regionName.split(',').map(p => p.trim());
    if (parts.length <= 1) return regionName;

    const countrySet = new Set(ronetDatacentersCache.flatMap(entry => {
        const loc = entry.location || entry;
...
export function createInfoElement(className, svg, text) {
    ...
    element.style.color = 'var(--ronet-main-text-color)';
    
    const iconSpan = document.createElement('span');
    iconSpan.className = 'ronet-icon-wrapper';
...
function updateInfoElement(container, type, iconHTML, text, isVisible = true) {
    ...
    const iconWrapper = element.querySelector('.ronet-icon-wrapper');
    const textWrapper = element.querySelector('span:not(.ronet-icon-wrapper)');
...
function injectStyles() {
    if (document.getElementById('ronet-dynamic-styles')) return;

    const style = document.createElement('style');
    style.id = 'ronet-dynamic-styles';
    style.textContent = `
...
function enableAvatarLinks(server) {
    ...
    avatarLinks.forEach(link => {
        if (link.dataset.ronetHooked) return;
        link.addEventListener('click', (e) => e.stopPropagation());
        link.dataset.ronetHooked = 'true';
    });
}
...
export function displayPerformance(server, fps, serverLocations = {}) {
    ...
    if (serverLocations[server.dataset.ronetServerid] === 'private') return;
...
export function displayIpAndDcId(server) {
    let extraDiv = server.querySelector('.ronet-server-extra-details');
    ...
    extraDiv.className = 'ronet-server-extra-details text-info xsmall';

    const ip = server.dataset.ronetIp;
    const dcId = server.dataset.ronetDcId;
...
export async function fetchServerUptime(placeId, serverIds, serverLocations, serverUptimes) {
    ...
    try {
        ...
        data.servers.forEach(info => {
            ...
            const serverEls = document.querySelectorAll(`[data-ronet-serverid="${server_id}"]`);
            
            serverEls.forEach(serverEl => {
                if (ip_address != null) serverEl.dataset.ronetIp = ip_address;
                if (datacenter_id != null) serverEl.dataset.ronetDcId = datacenter_id;
        ...
        validIds.filter(id => !foundIds.has(id)).forEach(id => {
            ...
            const matchingEls = document.querySelectorAll(`[data-ronet-serverid="${id}"]`);
...
export async function fetchAndDisplayRegion(server, serverId, serverIpMap, serverLocations, options = {}) {
    ...
    try {
        ...
        if (server.dataset.ronetServerid !== serverId) return;
        ...
        if (response.ok) {
            ...
            if (info.joinScript) {
                ...
                if (joinScript.DataCenterId != null && !server.dataset.ronetDcId) {
                    server.dataset.ronetDcId = joinScript.DataCenterId;
                    changed = true;
                }

                if (!server.dataset.ronetIp) {
                    let ip = null;
...
export function isExcludedButton(node) {
    ...
    return node.classList.contains('ronet-copy-join-link') ||
           node.classList.contains('ronet-vip-invite-link') ||
...
export function attachCleanupObserver(server) {
    ...
    observeElement('.share-button, .server-performance', (node) => {
        if (!isExcludedButton(node) && node.closest('[data-ronet-serverid]')) {
            node.remove();
        }
    }, { multiple: true });
}
...
export async function addCopyJoinLinkButton(server, serverId) {
    ...
    if (server.querySelector('.ronet-copy-join-link')) return;

    const placeId = server.dataset.placeid || window.location.href.match(/\/games\/(\d+)\//)?.[1];
    if (!placeId) return;

    const btn = document.createElement('button');
    btn.className = 'btn-full-width btn-control-xs btn-primary-md btn-min-width ronet-copy-join-link';
...
export async function enhanceServer(server, context) {
    ...
    if (!server._ronetListener) {
        server._ronetListener = () => enhanceServer(server, context);
        server.addEventListener('ronet-serverid-set', server._ronetListener);
    }

    let serverId = server.getAttribute('data-ronet-serverid');
    const isPrivate = server.classList.contains('rbx-private-game-server-item');
    ...
    const lastId = server._ronetLastProcessedId;
    if (lastId && lastId !== serverId) {
        server.dataset.ronetEnhanced = 'false';
        cleanupServerUI(server);
    } else if (server.dataset.ronetEnhanced === 'true' && lastId === serverId) {
        enableAvatarLinks(server);
    }

    server._ronetLastProcessedId = serverId;
    server.dataset.ronetEnhanced = 'true';
    
    server.classList.add('ronet-checked');
    ...
    const apiData = server._ronetApiData;
    ...
    if (isServerUptimeEnabled || isServerRegionEnabled || isPlaceVersionEnabled) {
        if (serverUptimes[serverId] === undefined && !isPrivate) {
            ...
            clearTimeout(server._ronetUptimeTimeout);
            server._ronetUptimeTimeout = setTimeout(() => processUptimeBatch(), 100);
    ...
    if (isFullServerIDEnabled && !isPrivate) {
        ...
        if (!idDiv) {
            ...
            const appendTarget = server.querySelector('.ronet-copy-join-link') || server.querySelector('.game-server-join-btn');
