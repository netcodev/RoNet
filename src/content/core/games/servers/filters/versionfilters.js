...
const STYLES = `
    ...
    .ronet-version-item:hover {
        background-color: var(--color-state-hover);
    }

    .ronet-version-item:active {
        background-color: var(--color-state-press);
    }
    
    .ronet-version-item span {
        font-weight: 600;
    }
`;
...
async function fetchVersionCounts() {
    try {
        ...
        console.warn('RoNet: Failed to fetch version counts', e);
        return [];
    }
}
...
async function fetchServersForVersion(version, cursor = null) {
    ...
    try {
        ...
        console.error(`RoNet: Failed to fetch version ${version}.`, error);
        return null;
    }
}
...
async function executeSearch(version) {
    ...
    document.dispatchEvent(new CustomEvent('ronetVersionSelected', { detail: { version } }));
...
async function createVersionWidget(container) {
    if (!document.getElementById('ronet-version-filter-styles')) {
        const s = document.createElement('style');
        s.id = 'ronet-version-filter-styles';
    ...
    dropdown.element.classList.add('ronet-version-filter-widget', 'ronet-filter-widget');
    ...
    if (dropdown.panel) {
        ...
        if (versions.length > 0) {
            ...
            listContainer.className = 'ronet-version-list';
            
            versions.forEach(v => {
                const item = document.createElement('div');
                item.className = 'ronet-version-item';
...
export function initVersionFilters() {
    ...
    document.addEventListener('ronetRegionSelected', resetHandler);
    document.addEventListener('ronetUptimeSelected', resetHandler);
    document.addEventListener('ronetClearFilters', resetHandler);

    observeElement('#ronet-main-controls', (container) => {
        if (container.querySelector('.ronet-version-filter-widget')) return;
