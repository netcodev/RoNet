...
const GLOBE_PANEL_ID = 'ronet-globe-panel';
const GLOBE_CONTAINER_ID = 'ronet-globe-container';
const GLOBE_TOOLTIP_ID = 'ronet-globe-tooltip';
const EASTER_EGG_TRIGGER_ID = 'ronet-easter-egg-trigger';
const HEADER_TITLE_ID = 'ronet-header-title';

const EVT_REGIONS_UPDATED = 'ronetRegionsUpdated';
const EVT_GLOBE_UPDATE_DATA = 'ronetGlobe_UpdateData';
const EVT_GLOBE_EASTER_EGG = 'ronetGlobeEasterEgg';
const EVT_GLOBE_EASTER_EGG_OFF = 'ronetGlobeEasterEggOff';
const EVT_INIT_GLOBE = 'initRonetGlobe';
const EVT_REGION_SELECTED = 'ronetRegionSelected';
const EVT_UPTIME_SELECTED = 'ronetUptimeSelected';
const EVT_CLEAR_FILTERS = 'ronetClearFilters';
const EVT_REGION_SERVERS_LOADED = 'ronetRegionServersLoaded';
const EVT_REQUEST_REGION_SERVERS = 'ronetRequestRegionServers';
const EVT_GLOBE_HOVER = 'ronetGlobeHover';
...
async function cacheFlag(countryCode) {
    ...
    try {
        ...
        console.warn('RoNet: Failed to cache flag for', code);
    }
}
...
function closeGlobalPanels() {
    ...
    document
        .querySelectorAll('.ronet-side-panel.show')
...
function createGlobePanel(container) {
    ...
    panel.innerHTML = `<div class="ronet-globe-header ${theme}"><img src="${assets.ronetIcon}" class="ronet-header-logo" title="RoNet" id="${EASTER_EGG_TRIGGER_ID}" alt="Logo"><div id="${HEADER_TITLE_ID}" style="font-weight:bold;">RoNet Region Selector</div></div><div id="${GLOBE_CONTAINER_ID}"></div>`;
            ...
            if (State.easterEggActive) {
                document.dispatchEvent(
                    new CustomEvent(EVT_GLOBE_EASTER_EGG, {
                        detail: { iconUrl: assets.ronetIcon },
                    }),
                );
                ...
                if (title) title.textContent = 'RoNet Region Selector';
            }
        }
    });
...
function populateRegionSidePanel(container, theme) {
    ...
    for (const [groupName, items] of Object.entries(groups)) {
        ...
        header.className = `ronet-filter-group-header ${theme}`;
        header.textContent = groupName;
        container.appendChild(header);
        items.forEach((item) => {
            const row = document.createElement('a');
            row.className = `ronet-side-panel-item ${theme}`;
            row.dataset.regionCode = item.code;
            row.innerHTML = DOMPurify.sanitize(
                `<div><strong>${item.label}</strong><span class="country"> ${item.subLabel}</span></div><div class="ronet-region-count ${theme}">${item.count}</div>`,
            );
...
function createRegionDropdownWidget(container) {
    ...
    wrapper.id = 'ronet-region-filter-dropdown-wrapper';
    wrapper.className = 'filter-dropdown-container ronet-filter-widget';
    ...
    sidePanel.className = `ronet-side-panel ${theme}`;
    sidePanel.innerHTML = `<div class="ronet-side-panel-list ${theme}"></div>`;

    const listContainer = sidePanel.querySelector('.ronet-side-panel-list');
...
async function onRegionSelected(event) {
    ...
    const item = document.querySelector(
        `.ronet-side-panel-item[data-region-code="${code}"]`,
    );
...
async function onRequestMoreRegionServers(event) {
    ...
    const item = document.querySelector(
        `.ronet-side-panel-item[data-region-code="${regionCode}"]`,
    );
    if (item) item.classList.add('loading');
...
function onStorageChanged(changes, area) {
    if (area === 'local' && changes.ronetDatacenters) {
        processStorageDatacenters(changes.ronetDatacenters.newValue);
    }
}

async function initializeData() {
    const { ronetDatacenters } =
        await chrome.storage.local.get('ronetDatacenters');
    processStorageDatacenters(ronetDatacenters);
    await fetchCounts();
...
function setupUI() {
    ...
    const mainControls = document.getElementById('ronet-main-controls');
    if (
        mainControls &&
        !mainControls.querySelector('#ronet-region-filter-dropdown-wrapper')
    ) {
...
export async function initRegionFilters() {
    ...
    try {
        ...
        console.error('RoNet: Failed to start observing.', e);
    }
