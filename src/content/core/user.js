...
export async function getAuthenticatedUserId() {
    const storage = await chrome.storage.local.get('ronet_authed_user_id');
    const cachedId = storage.ronet_authed_user_id;
    ...
    const scrapeId = () => {
        ...
        if (actualId !== cachedId) {
            chrome.storage.local.set({ ronet_authed_user_id: actualId });
        }
        return actualId;
    };
