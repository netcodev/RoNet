const injectScrollbarStyles = () => {
    if (document.getElementById('ronet-dropdown-styles')) return;
    const style = document.createElement('style');
    style.id = 'ronet-dropdown-styles';
    style.textContent = `
        .ronet-no-scrollbar::-webkit-scrollbar { display: none; }
        .ronet-no-scrollbar { -ms-overflow-style: none; scrollbar-width: none; }
        .ronet-dropdown-item[data-selected="true"].highlight-enabled {
            background-color: rgba(255, 255, 255, 0.1) !important;
            border-left: 3px solid var(--ronet-play-button-color); 
        }
        .ronet-text-clamp-2 {
...
export function createDropdownContent(
    ...
    contentPanel.className = 'ronet-dropdown-content-panel foundation-web-menu bg-surface-100 stroke-standard stroke-default shadow-transient-high radius-large';
    ...
    if (!showScrollbar) {
        dropdownContentInner.classList.add('ronet-no-scrollbar');
    }
    ...
    const updateSelectedState = (newValue) => {
        ...
        contentPanel.querySelectorAll('.ronet-dropdown-item').forEach(el => {
    ...
    if (hasGroups) {
        ...
        continentOrder.forEach(groupName => {
            if (grouped[groupName]) {
                ...
                headerEl.className = 'ronet-dropdown-section-header';
    ...
    function createDropdownItem(item) {
        ...
        itemEl.className = 'ronet-dropdown-item relative clip group/interactable focus-visible:outline-focus disabled:outline-none foundation-web-menu-item flex items-center content-default text-truncate-split focus-visible:hover:outline-none cursor-pointer stroke-none bg-none text-align-x-left width-full text-body-medium padding-x-medium padding-y-small gap-x-medium radius-medium';
        itemEl.type = 'button';
