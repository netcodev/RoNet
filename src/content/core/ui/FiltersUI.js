...
function injectStyles() {
    if (document.getElementById('ronet-filters-ui-styles')) return;
    const style = document.createElement('style');
    style.id = 'ronet-filters-ui-styles';
    style.textContent = `
        #ronet-fx-dropdown {
            display: none;
            position: absolute;
        ...
        #ronet-fx-dropdown[data-state="open"] {
            display: block;
        }
    `;
...
export function createAvatarFilterUI({ avatarFiltersEnabled, searchbarEnabled, onApply, onSearch, filterConfig = [] }) {
    ...
    if (avatarFiltersEnabled) {
        ...
        toggleButton.id = 'ronet-fx-toggle-btn';
        toggleButton.type = 'button';
        toggleButton.className = 'ronet-dropdown-trigger relative clip group/interactable focus-visible:outline-focus disabled:outline-none flex items-center justify-between width-full bg-none stroke-standard radius-medium height-1000 padding-x-medium text-body-medium stroke-default content-default';
        ...
        chevron.className = 'ronet-dropdown-chevron size-500 icon icon-regular-chevron-large-down content-default';

        const dropdown = document.createElement('div');
        dropdown.id = 'ronet-fx-dropdown';
        dropdown.className = 'ronet-dropdown-content foundation-web-menu bg-surface-100 stroke-standard stroke-default shadow-transient-high radius-large';
        ...
        applyBtn.id = 'ronet-price-apply-btn';
    ...
    if (searchbarEnabled) {
        ...
        searchInput.id = 'ronet-fx-search-bar';
