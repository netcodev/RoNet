...
function getOnlineFriends(userId) {
    if (!friendCachePromise) {
        friendCachePromise = callRobloxApi({
            ...
            console.warn('RoNet: Friend fetch error', e);
            return { data: [] };
        });
...
export function createGameCard(options) {
    ...
    if (!game && (gameId || placeId)) {
        ...
        card.className = 'ronet-game-card';
        card.innerHTML = `
            ...
            try {
                ...
                if (friendsData) {
                    try {
                        ...
                        console.warn('RoNet: Error fetching friend info', e);
                    }
                }
                ...
                console.warn('RoNet: Error creating game card from ID', e);
