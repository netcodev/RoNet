...
function injectDropdownCss() {
    ...
    style.id = 'ronet-global-dropdown-style';
    style.textContent = `
        ...
        .ronet-dropdown-trigger .ronet-dropdown-chevron {
            transition: transform 0.2s ease !important;
        }
        .ronet-dropdown-trigger[data-state="open"] .ronet-dropdown-chevron {
            transform: rotate(180deg);
        }
        .ronet-dropdown-panel {
            cursor: default;
        }
    `;
...
export function createDropdown({ items = [], initialValue, placeholder = 'Select...', onValueChange, showFlags = false }) {
    ...
    container.className = 'ronet-dropdown-container';

    const trigger = document.createElement('button');

    trigger.className = 'ronet-dropdown-trigger relative clip group/interactable focus-visible:outline-focus disabled:outline-none flex items-center justify-between width-full bg-none stroke-standard radius-medium height-1000 padding-x-medium text-body-medium stroke-default content-default';
    trigger.type = 'button';
