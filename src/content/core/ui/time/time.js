const TIME_FORMAT_KEY = 'ronet_time_format_preference';
...
chrome.storage.onChanged.addListener((changes, area) => {
    if (area === 'local' && changes[TIME_FORMAT_KEY]) {
        ...
        if (FORMATS.includes(newFormat) && newFormat !== preferredFormat) {
            ...
            document.dispatchEvent(new CustomEvent('ronet-time-format-change', { detail: { format: newFormat } }));
        }
    }
});
...
export function createInteractiveTimestamp(dateString) {
    ...
    container.className = 'ronet-interactive-timestamp';
    container.style.position = 'relative';
    container.style.cursor = 'pointer';

    const timeSpan = document.createElement('span');
    timeSpan.style.borderBottom = '1px dashed color-mix(in srgb, var(--ronet-secondary-text-color) 50%, transparent)';
    ...
    document.addEventListener('ronet-time-format-change', handleFormatChange);
    ...
    container.addEventListener('click', (e) => {
        ...
        document.dispatchEvent(new CustomEvent('ronet-time-format-change', {
    ...
    const observer = new MutationObserver(() => {
        if (!document.body.contains(container)) {
            if (updateInterval) clearInterval(updateInterval);
            document.removeEventListener('ronet-time-format-change', handleFormatChange);
            observer.disconnect();
        }
    });
