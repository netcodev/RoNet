...
export function init() {
    ...
    const BANNER_ID = 'ronet-catalog-notice-banner';
    ...
    if (!window.CatalogBannerManager) {
        window.CatalogBannerManager = {
            addNotice: function (title, iconHtml = '', description = '') {
                ...
                entry.className = 'ronet-catalog-notice-entry';
                ...
                if (iconHtml) {
                    ...
                    if (svgElement) {
                        ...
                        iconContent = `<div class="ronet-catalog-notice-icon" style="display: flex; flex-shrink: 0; align-items: center;">${modifiedIconHtml}</div>`;
                    } else if (imgElement) {
                        ...
                        if (styleWidth > 48 || attrWidth > 48) {
                            banner.classList.add('ronet-banner-compact');
                        }

                        iconContent = `<div class="ronet-catalog-notice-icon" style="display: flex; flex-shrink: 0; align-items: center;">${tempDiv.innerHTML}</div>`;
                    }
                }

                const textContainer = document.createElement('div');
                textContainer.className = 'ronet-catalog-notice-text-container';
                textContainer.style.flex = '1';

                const titleDiv = document.createElement('div');
                titleDiv.className = 'ronet-catalog-notice-title';
                titleDiv.innerHTML = parsedTitle; // Verified

                titleDiv.style.fontSize = fontSize;
                titleDiv.style.fontWeight = description ? '600' : '400';

                const mdWrapper = titleDiv.querySelector('.ronet-markdown');
                if (mdWrapper) {
                ...
                if (description) {
                    ...
                    descDiv.className = 'ronet-catalog-notice-description';
                    descDiv.innerHTML = parsedDescription; // Verified

                    const descWrapper =
                        descDiv.querySelector('.ronet-markdown');
