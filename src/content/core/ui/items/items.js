...
async function processBatch() {
    ...
    try {
        ...
        if (!looksRes.ok) console.warn('RoNet: Looks API request failed, prices may be incomplete.');
        ...
        console.warn('RoNet: Batch request failed', e);
        currentBatch.forEach(request => {
...
export function createItemCard(itemOrId, thumbnailCacheOrConfig, config = {}) {
    if (typeof itemOrId === 'number' || typeof itemOrId === 'string') {
        ...
        card.className = 'ronet-item-card';
        ...
        card.innerHTML = `
            <div class="ronet-item-thumb-container shimmer" style="width: 100%; height: 150px; border-radius: 8px; margin-bottom: 4px;"></div>
            <div class="ronet-item-name shimmer" style="height: 14px; width: 90%; margin-bottom: 4px; border-radius: 4px;"></div>
            <div class="ronet-item-rap shimmer" style="height: 14px; width: 60%; border-radius: 4px;"></div>
        `;
    ...
    if (item.price !== undefined && item.price !== null) {
        card.dataset.ronetPrice = item.price;
        if (item.bundleId) {
            card.dataset.ronetBundleId = item.bundleId;
        }
    }
    ...
    thumbContainer.className = 'ronet-item-thumb-container';
    ...
    if (showOnHold && item.isOnHold) {
        ...
        onHoldIconElement.className = 'ronet-on-hold-icon-container';
    ...
    card.innerHTML = `
        <a href="${itemUrl}" class="ronet-item-card-link">
            <div class="ronet-item-name"></div>
            <div class="ronet-item-rap">
                ${priceHtml}
            </div>
        </a>
    `; //Verified

    const nameDiv = card.querySelector('.ronet-item-name');
