...
function injectFileUploadCss() {
    ...
    style.id = 'ronet-global-fileupload-style';
    style.textContent = `
        .ronet-fileupload-wrapper {
            display: flex;
        ...
        .ronet-fileupload-controls {
            display: flex;
            align-items: center;
            gap: 8px;
        }

        .ronet-fileupload-filename {
            font-size: 12px;
        ...
        .ronet-fileupload-preview.visible {
            display: flex;
        }
        .ronet-fileupload-preview img {
            max-width: 100%;
            max-height: 150px;
            border-radius: 4px;
            object-fit: contain;
        ...
        .ronet-fileupload-size {
            font-size: 11px;
            color: var(--text-secondary);
        }
        .ronet-fileupload-error {
            font-size: 11px;
...
export function createFileUpload({ id, accept = 'image/*', compress = true, compressSettingName, onFileSelect, onFileClear } = {}) {
    ...
    wrapper.className = 'ronet-fileupload-wrapper';

    const controlsContainer = document.createElement('div');
    controlsContainer.className = 'ronet-fileupload-controls';
    ...
    triggerButton.className = 'ronet-fileupload-trigger relative clip group/interactable focus-visible:outline-focus disabled:outline-none flex items-center justify-between width-full bg-none stroke-standard radius-medium height-1000 padding-x-medium text-body-medium stroke-default content-default';
    ...
    fileNameDisplay.className = 'ronet-fileupload-filename';
    fileNameDisplay.style.display = 'none';

    const clearButton = document.createElement('button');
    clearButton.type = 'button';
    clearButton.id = `${id}-clear`;
    clearButton.className = 'btn-control-xs ronet-fileupload-clear';
    clearButton.innerHTML = '<span class="icon-close"></span>';
    clearButton.style.display = 'none';

    const previewContainer = document.createElement('div');
    previewContainer.className = 'ronet-fileupload-preview';
    
    const previewImage = document.createElement('img');
    previewImage.alt = 'File preview';
    
    const fileSizeDisplay = document.createElement('span');
    fileSizeDisplay.className = 'ronet-fileupload-size';
    
    previewContainer.append(previewImage, fileSizeDisplay);

    const errorMessage = document.createElement('span');
    errorMessage.className = 'ronet-fileupload-error';
    ...
    wrapper.ronetFileUpload = componentApi;
