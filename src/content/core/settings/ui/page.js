...
export async function checkRoNetPage() {
    const urlParams = new URLSearchParams(window.location.search);
    const ronetTab = urlParams.get('ronet');
    ...
    document.body.classList.add('ronet-settings-loading');
    ...
    async function handleHashChange() {
        const urlParams = new URLSearchParams(window.location.search);
        const ronetTabFromParam = urlParams.get('ronet');
        const hashPart = decodeURIComponent(window.location.hash.replace('#!/', '').replace('#!', ''));
        const currentHash = hashPart || ronetTabFromParam || 'info';
    ...
    const { ronetHeader, settingsContainer } = await buildSettingsPage({
    ...
    if (ronetHeader && settingsContainer) {
        const unifiedMenu = document.getElementById('unified-menu');
        await loadTabContent(ronetTab || 'info');
        await applyTheme();
    }
}