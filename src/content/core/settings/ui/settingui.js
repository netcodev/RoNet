...
export async function buildSettingsPage({ handleSearch, debounce, loadTabContent, buttonData, REGIONS, initSettings }) {
    ...
    if (!containerMain) {
        console.error("RoNet: Main container not found. Cannot build settings page.");
        return {};
    }
    ...
    let ronetIcon = document.createElement('img');
    ronetIcon.src = assets.ronetIcon;
    ronetIcon.style.cssText = 'width: 35px; height: 35px; margin-left: 5px;  user-select: none;'; 

    let ronetHeader = document.createElement('h1');
    ronetHeader.textContent = 'RoNet Settings';
    ronetHeader.style.margin = '0';
    ronetHeader.style.color = 'var(--ronet-main-text-color)'; 

    headerContainer.appendChild(ronetHeader);
    ronetHeader.appendChild(ronetIcon);
    ...
    mobileMenuContainer.id = 'ronet-mobile-menu-container';
    ...
    const renderMobileDropdown = () => {
        ...
        const initialTab = urlParams.get('ronet') || 'info';
        const dropdownItems = [];
        ...
        const mobileDropdown = createDropdown({
            ...
            onValueChange: async (value) => {
                ...
                if (newUrl.searchParams.get('ronet') !== value) {
                    newUrl.searchParams.set('ronet', value);
    ...
    uiContainer.id = 'ronet-ui-container'; 
    ...
    settingsContainer.insertAdjacentElement("afterbegin", ronetHeader);
    ...
    ronetIcon.addEventListener('click', () => {
        let ronetIconClickCount = (ronetIcon.dataset.clickCount || 0) * 1;
        ronetIconClickCount++;
        ronetIcon.dataset.clickCount = ronetIconClickCount;

        if (ronetIconClickCount >= 10 && !devTabAdded) {
            devTabAdded = true;
    ...
    return { ronetHeader, settingsContainer, contentDiv, userAccountDiv };
}
...
function createUnifiedMenu({ handleSearch, debounce, buttonData, devTabAdded, loadTabContent, REGIONS, initSettings }) {
    ...
    menuList.className = 'menu-vertical ronet-sidebar';
    ...
    searchInput.style.cssText = 'width: 89%; padding: 8px; border-radius: 0px; font-size: 14px; border: 0px solid var(--ronet-container-background-color) !important; background: transparent !important; color: var(--ronet-main-text-color) !important;';
    ...
    const performSearch = debounce((query) => {
        try {
            ...
            console.warn("RoNet: Search handler failed:", error);
        }
    }, 300);
    ...
    searchInput.addEventListener('focus', () => {
        ...
        const newUrl = new URL(window.location.href);
        if (newUrl.searchParams.get('ronet') !== 'search') {
            newUrl.searchParams.set('ronet', 'search');
            history.pushState(null, '', newUrl.pathname + newUrl.search);
        }
    });
    ...
    staticItems.forEach(item => {
        ...
        link.addEventListener('click', async (e) => {
            ...
            const newUrl = new URL(window.location.href);
            if (newUrl.searchParams.get('ronet') !== newHashKey) {
                newUrl.searchParams.set('ronet', newHashKey);
                history.pushState(null, '', newUrl.pathname + newUrl.search);
            }
            await loadTabContent(newHashKey);
            stripInlineStyles(document.getElementById('content-container'));
            
            const dropdownTrigger = document.querySelector('#ronet-mobile-menu-container .ronet-dropdown-trigger span');
    ...
    separator.style.cssText = 'height: 1px; background-color: var(--ronet-secondary-text-color); opacity: 0.3; margin: 10px 0;';
...
function createSidebarItem(sectionName, title, loadTabContent) {
    ...
    link.addEventListener('click', async function(e) {
        ...
        const newUrl = new URL(window.location.href);
        if (newUrl.searchParams.get('ronet') !== sectionName.toLowerCase()) {
            newUrl.searchParams.set('ronet', sectionName.toLowerCase());
            history.pushState(null, '', newUrl.pathname + newUrl.search);
        }
