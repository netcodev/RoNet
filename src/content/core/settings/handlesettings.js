...
const syncToSettingsKey = (settingName, value) => {
    chrome.storage.local.get('ronet_settings', (result) => {
        const settingsData = result.ronet_settings || {};
        settingsData[settingName] = value;
        chrome.storage.local.set({ ronet_settings: settingsData });
    });
};
    ...
    return new Promise((resolve) => {
        ...
        chrome.storage.local.get(allSettingKeys, (currentSettings) => {
            chrome.storage.local.set(
                { ronet_settings: currentSettings },
                () => {
                    if (chrome.runtime.lastError) {
                        console.error(
                        ...
                        console.log('RoNet: Settings key initialized');
                    }
                    resolve();
                },
            );
        ...
        for (const sectionName in SETTINGS_CONFIG) {
            ...
            for (const [settingName, config] of Object.entries(
                ...
                if (
                    ...
                    if (missingPerms) {
                        console.log(
                            `RoNet: Disabling '${settingName}' because required permissions are missing.`,
                        );
            ...
            for (const [settingName, setting] of Object.entries(
                ...
                if (element) {
                    if (setting.type === 'checkbox') {
                        ...
                        if (
                            fileUploadWrapper &&
                            fileUploadWrapper.ronetFileUpload
                        ) {
...
export function updateConditionalSettingsVisibility(
    ...
    numberToggles.forEach((toggle) => {
        ...
        const numberInputContainer = settingsContent
            ...
            ?.closest('.ronet-number-input-container');
    ...
    return new Promise((resolve) => {
        chrome.runtime.sendMessage(
            ...
            (response) => {
                if (chrome.runtime.lastError) {
                    console.error(
                        'RoNet: Error checking permission:',
    ...
    return new Promise((resolve) => {
        chrome.runtime.sendMessage(
            ...
            (response) => {
                if (chrome.runtime.lastError) {
                    console.warn(
                        `RoNet: Permission request for '${permission}' failed or was dismissed:`,
                        chrome.runtime.lastError.message,
                    );
                    resolve(false);
                }
    ...
    return new Promise((resolve) => {
        chrome.runtime.sendMessage(
            ...
            (response) => {
                if (chrome.runtime.lastError) {
                    console.error(
                        `RoNet: Failed to revoke '${permission}' permission:`,
                        chrome.runtime.lastError.message,
                    );
                    resolve(false);
                }
...
export function initializeSettingsEventListeners() {
    document.addEventListener('ronet:open40methodSetup', () => {
        createAndShowPopup(() => {
            // Configuration is saved by the popup.
        });
    });

    document.addEventListener('ronet:generateEnvironmentJson', async () => {
        const settings = await loadSettings();
        const envConfig = {};
                ...
                if (settingConfig?.requiredPermissions) {
                    ...
                    if (missingPermissions.length > 0) {
                        ...
                        if (!granted) {
                            ...
                            console.log(
                                `RoNet: Permission denied for ${settingName}`,
                            );
                            return;
                        }
                    }
    ...
    document.addEventListener('click', (event) => {
        ...
        const numberButton = target.closest('.ronet-number-input-btn');
        if (numberButton) {
        ...
        if (target.id?.startsWith('clear-')) {
            ...
            if (settingName) {
                ...
                if (fileUploadWrapper && fileUploadWrapper.ronetFileUpload) {
