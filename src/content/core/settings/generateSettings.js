            ...
            onConfirm: () => {
                chrome.storage.local.remove(storageKey, () => {
                    if (settingType === 'file' && inputElement) {
                         const uploadApi = inputElement._uploadApi || inputElement.ronetFileUpload;
                         if (uploadApi) {
...
export function generateSettingInput(settingName, setting, REGIONS = {}) {
    ...
    if (setting.type === 'checkbox') {
        ...
        const wrapper = document.createElement('div');
        wrapper.className = 'ronet-number-input-wrapper';
        wrapper.style.cssText = 'display: flex; align-items: center; gap: 12px; margin-left: auto;';
        wrapper.innerHTML = DOMPurify.sanitize(`
            <div class="ronet-number-input-container" style="display: flex; align-items: center; gap: 8px; background-color: var(--ronet-container-background-color); padding: 4px; border-radius: 8px;">
                <button type="button" class="ronet-number-input-btn btn-control-xs" data-action="decrement" data-target="${settingName}" style="width: 32px; height: 32px; padding: 0; line-height: 0; border: none;">
                    <svg class="MuiSvgIcon-root MuiSvgIcon-fontSizeMedium css-1phnduy" focusable="false" aria-hidden="true" viewBox="0 0 24 24" style="width: 20px; height: 20px; fill: var(--ronet-main-text-color);"><path d="M19 13H5v-2h14z"></path></svg>
                </button>
                <input type="number" id="${settingName}" data-setting-name="${settingName}" class="setting-number-input" 
                       ...
                       style="width: 60px; text-align: center; -moz-appearance: textfield; appearance: textfield; border-radius: 6px; border: 1px solid var(--ronet-border-color); background-color: var(--ronet-main-background-color); color: var(--ronet-main-text-color); padding: 8px; font-weight: 500;">
                <button type="button" class="ronet-number-input-btn btn-control-xs" data-action="increment" data-target="${settingName}" style="width: 32px; height: 32px; padding: 0; line-height: 0; border: none;">
                    <svg class="MuiSvgIcon-root MuiSvgIcon-fontSizeMedium css-1phnduy" focusable="false" aria-hidden="true" viewBox="0 0 24 24" style="width: 20px; height: 20px; fill: var(--ronet-main-text-color);"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6z"></path></svg>
                </button>
            </div>
...
export function generateSingleSettingHTML(settingName, setting, REGIONS = {}) {
    ...
    if (setting.requiredPermissions && setting.requiredPermissions.length > 0) {
        ...
        permissionManager.style.cssText = 'margin-top: 10px; padding: 10px; background-color: var(--ronet-container-background-color, rgba(0,0,0,0.1)); border-radius: 8px;';
        ...
        text.style.cssText = 'font-size: 15px; color: var(--ronet-main-text-color); font-weight: 400;';
    ...
    if (setting.type === 'file') {
        ...
        const uploadApi = uploadElement._uploadApi || uploadElement.ronetFileUpload;
