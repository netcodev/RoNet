...
function executeLaunchScript(codeToInject) {
    if (typeof chrome !== 'undefined' && chrome.runtime) {
        ...
        console.error("RoNet Launcher: Chrome runtime is not available to inject the script.");
    }
}
...
export function launchMultiplayerGame(placeId, launchData = {}) {
    window.__ronet_skipNextLaunch = true;
...
export async function launchStudioForGame(placeId) {
    try {
        ...
        console.error('RoNet Launcher: Failed to launch studio with universeId, falling back.', error);
