...
export async function checkAssetsInBatch(assetIds) {
    ...
    try {
        ...
        if (!batchApiResponse.ok) {
            console.error(`[Ronet Asset Parser] AssetDelivery batch API failed: ${batchApiResponse.status}`);
        ...
        const processingPromises = assetIds.map(async (id) => {
            ...
            try {
                ...
                console.error(`[Ronet Asset Parser] Error parsing asset ${id}:`, error);
        ...
        console.error('[Ronet Asset Parser] Critical error:', error);
        return assetIds.map(id => createDefaultResult(id));
    }
}