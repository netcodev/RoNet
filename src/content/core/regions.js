...
const STORAGE_KEY_DATACENTERS = 'ronetDatacenters';
...
export async function loadDatacenterMap() {
    if (window.ronetDatacenterState && window.ronetDatacenterState !== 'initial') return;
    window.ronetDatacenterState = 'loading';
    ...
    try {
        ...
        console.error("RoNet: Error reading datacenter map from storage.", e);
    }
    ...
    if (!currentData) {
        try {
            ...
            console.error("RoNet: Could not load local fallback JSON.", e);
            serverIpMap = {};
        }
    }
    ...
    try {
        ...
        window.ronetDatacenterState = 'complete';
    }
}
...
async function fetchAndProcessRegions() {
    ...
    try {
        ...
        console.warn("RoNet: API failed, using fallback.", error.message);
        try {
            const fallbackUrl = getAssets().serverListJson;
            const response = await fetch(fallbackUrl);
            if (!response.ok) throw new Error(`Status ${response.status}`);
            ...
            console.error("RoNet Critical: Could not load region data.", fallbackError);
