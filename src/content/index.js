...
import { init as initRonetBadges } from './features/profile/header/RoNetBadges.js';
      ...
      if (route.features && Array.isArray(route.features)) {
        route.features.forEach((init) => {
          try {
            ...
            console.error('RoNet: Feature init failed', error);
          }
        });
      }
    }
...
async function initializePage() {
  ...
  console.log(`%cRoNet Initialized`, 'font-size: 1.5em; color: #FF4500;', `(Observer: ${observerStatus})`);
}
...
function handleUrlChange() {
  ...
  if (currentPath !== lastPath) {
    console.log(`%cRoNet: URL changed from ${lastPath} to ${currentPath}`, 'color: #FF4500;');
